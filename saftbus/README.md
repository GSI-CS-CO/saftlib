...
saftlib was originally written with D-Bus for inter process communication.
...
The driver class encapsulates a random number generator for integers in the range [1..6]. Lets call it "Dice". Proxies can create a random number using the method "Throw". The result is communicated using the signal "Result". All Proxies with a connected callback function will be notified when a new number was generated.
