...
template <size_t MAX_CHUNCKS, size_t CHUNCKSIZE> class ChunckAllocatorRT {
  // TODO: add documentation
public:
  ChunckAllocatorRT() : allocated_chuncks(0) {
  ...
  void free(char *ptr) {
    ...
    // OLD_DEBUG: std::cerr << "swapping " << a_idx << " <-> " << b_idx <<
    // std::endl;
    std::swap(a_idx, b_idx);
    // OLD_DEBUG: std::cerr << "swapping " << a_backidx << " <-> " << b_backidx
    // << std::endl;
  ...
  bool contains(char *ptr) {
    return (ptr >= chuncks[0].buffer && ptr <= chuncks[MAX_CHUNCKS].buffer);
  }
  bool full() { return allocated_chuncks == MAX_CHUNCKS; }
  bool fits(size_t n) { return n <= CHUNCKSIZE; }
