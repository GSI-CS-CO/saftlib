...
struct Service::Impl {
  ...
  uint64_t object_id;
  std::function<void()>
      destruction_callback; // a funtion can be attatched here that is called
...
struct Container::Impl {
  ...
  std::map<unsigned, std::unique_ptr<Service>>
      objects; // Container owns the Service objects
  std::map<std::string, unsigned>
      object_path_lookup_table; // maps object_path to saftbus_object_id
  std::vector<Service *> removed_services;
  std::map<std::string, std::function<std::string(void)>>
      additional_info_callbacks; // allow plugins to add additional info to be
                                 // shown by "saftbus-ctl -s"
  void reset_children_first(const std::string &object_path) {
    if (object_path == "/saftbus")
      return;
    bool found_child = false;
    for (auto &obj : objects) {
      if (obj.second && obj.second->d->object_path.find(object_path) == 0 &&
  ...
  void clear() {
    // Make sure that service objects are destroyed in the opposite order
    // (youngest object first). The std::map "objects" is sorted after object
    // id, which is increasing for all created objects. starting with the last
    // object in the map, the correct destruction order is assured. the lowest
    // object_id is 1 and /saftbus has it.
...
int Service::get_owner() { return d->owner; }
bool Service::is_owned() { return d->owner != -1; }
void Service::set_owner(int owner) { d->owner = owner; }
void Service::release_owner() { d->owner = -1; }
...
// Generate the mapping from interface_name to interface_no for all
// interface_names, return true if all interface_names are found, false
// otherwise
bool Service::get_interface_name2no_map(
  ...
  // If yes, return a map of interface_name -> interface_no for this particular
  // service object
  ...
  for (auto &interface_name : interface_names) {
    ...
    for (unsigned i = 0; i < get_interface_names().size(); ++i) {
      if (interface_name == get_interface_names()[i]) {
        interface_name2no_map[interface_name] =
            i; // for this particular service object, i is the interface_no for
               // interface_name
...
void Service::emit(Serializer &send) {
  for (auto &fd_use_count_dropped :
       d->signal_fds_use_count_and_dropped_signals) {
    ...
    if (use_count > 0) { // only send data if use count is > 0
      ...
      pfd.events = POLLOUT;
      if (poll(&pfd, 1, 0) <=
          0) {             // fd is not ready immediately => drop the signal
        ++dropped_signals; // count number of dropped signals (this number can
                           // be seen when with "saftbus-ctl -s")
      } else {
        send.write_to_no_init(
            fd); // The same data is written multiple times. Therefore the
...
int Service::get_object_id() { return d->object_id; }

std::string &Service::get_object_path() { return d->object_path; }
...
Container_Service::Container_Service(Container *instance,
                                     std::function<void()> destruction_callback)
    : saftbus::Service(gen_interface_names(), destruction_callback),
      d(instance) {}
Container_Service::~Container_Service() {}
void Container_Service::call(unsigned interface_no, unsigned function_no,
                             int client_fd, saftbus::Deserializer &received,
                             saftbus::Serializer &send) {
  try {
    switch (interface_no) {
    case 0: // Container
      switch (function_no) {
      case 0: { // Container::register_proxy (Hand-written. It will be called by
                // Poxy base class constructor)
        std::string object_path;
        ...
        unsigned saftbus_object_id =
            d->register_proxy(object_path, interface_names,
                              interface_name2no_map, client_fd, signal_fd);
        send.put(saftbus_object_id);
        send.put(client_fd); // fd and signal_fd are used in the proxy
                             // de-registration process
        send.put(signal_fd); // send the integer value of the signal_fd back to
                             // the proxy. This nuber can be used by other
                             // Proxies to reuse the signal pipe.
        send.put(interface_name2no_map);
      }
        return;
      case 1: { // Container::unregister_proxy (Hand-written. It will be called
                // by Proxy base class destructor)
        unsigned saftbus_object_id;
        ...
        d->unregister_proxy(saftbus_object_id, received_client_fd,
                            received_signal_group_fd);
      }
        return;
      ...
      case 3: { // Container::unload_plugin
        ...
        return;
      case 4: { // Container::remove_object  // this is largely hand written,
                // because we have to make sure that the destruction callback is
                // correctly executed
        std::string object_path;
        received.get(object_path);
        ...
        if (d->d->object_path_lookup_table.find(object_path) !=
            d->d->object_path_lookup_table
                .end()) { // it may be that destruction callback already removed
                          // the object
...
Container::Container(ServerConnection *connection) : d(new Impl) {
  unsigned object_id =
      create_object("/saftbus", std::move(std::unique_ptr<Container_Service>(
                                    new Container_Service(this))));
  assert(
      object_id ==
      1); // the entier system relies on having Container_Service at object_id 1
...
unsigned Container::create_object(const std::string &object_path,
                                  std::unique_ptr<Service> service) {
  if (d->object_path_lookup_table.find(object_path) !=
      d->object_path_lookup_table.end()) {
  ...
  auto insertion_result =
      d->objects.insert(std::make_pair(saftbus_object_id, std::move(service)));
  ...
  if (insertion_took_place) {
    inserted_object->d->object_path =
        object_path; // set the object_path of the Service object
...
Service *Container::removal_helper(const std::string &object_path) {
  if (object_path == "/saftbus") {
    throw saftbus::Error(saftbus::Error::INVALID_ARGS,
                         "cannot remove /saftbus");
  }
  auto find_result = d->object_path_lookup_table.find(object_path);
  if (find_result == d->object_path_lookup_table.end()) {
  ...
  if (service->d->owner != -1) { // the service is owned
    if (service->d->owner != d->connection->get_calling_client_id()) {
      ...
      msg << "cannot remove object \"" << object_path
          << "\" because it owned by other client " << service->d->owner;
      throw saftbus::Error(saftbus::Error::INVALID_ARGS, msg.str());
    }
  }

  // make sure that non of the children of the object to be removed has a
  // foreign owner the child relationship is determined only based on the object
  // path it is in the responability of the Service developer that all child
  ...
  for (auto &object : d->object_path_lookup_table) {
    ...
    if (other_path != object_path && // check if other_path differs
        other_path.find(object_path) ==
            0 && // check if other_path starts with object_path
        other_path.size() >
            object_path
                .size() && // check if other_path is longer then object_path
                           // (child object paths are always longer)
        other_path[object_path.size()] ==
            '/') { // and the first non-commom character in a child path is '/',
                   // as in "/parent/child".
      ...
      if (service->d->owner != -1) { // the service is owned
        if (service->d->owner != d->connection->get_calling_client_id()) {
          std::ostringstream msg;
          msg << "cannot remove object \"" << object_path
              << "\" because child object \"" << other_path
              << "\" is owned by other client " << service->d->owner;
          throw saftbus::Error(saftbus::Error::INVALID_ARGS, msg.str());
        }
      }
    }
...
int Container::register_proxy(const std::string &object_path,
                              const std::vector<std::string> interface_names,
                              std::map<std::string, int> &interface_name2no_map,
  ...
  if (find_result != d->object_path_lookup_table.end()) {
    ...
    assert(find_result != d->objects.end()); // if this cannot be found, the
                                             // lookup table is not correct
    auto &service = find_result->second;
    if (service->get_interface_name2no_map(
            interface_names,
            interface_name2no_map)) { // returns false if not all requested
                                      // interfaces are implemented
      service->d->signal_fds_use_count_and_dropped_signals[signal_group_fd]
          .first++;
...
void Container::unregister_proxy(unsigned saftbus_object_id, int client_fd,
                                 int signal_group_fd) {
  auto find_result = d->objects.find(saftbus_object_id);
  if (find_result == d->objects.end()) {
    // std::cerr << "object id " << saftbus_object_id << " already gone" <<
    // std::endl;
    return;
  }
  auto &service = find_result->second;
...
bool Container::call_service(unsigned saftbus_object_id, int client_fd,
...
// operator is used to std::find a Service based on owner, where the owner is
// identified by file descriptor (fd) of the client socket
bool operator==(
...
void Container::client_hung_up(int fd) {
  // There may be parent-child relations between service objects it must be
  // ensured that children are always destroyed before their parents. All
  // service objects get an object id which is always increasing. Thus, children
  // always have larger numbers then their parents. Thus a viable strategy to
  // remove children before parents is the use of reverse iterators when
  // selecting the owned service objects. This works because iterating a map
  // with object ids as key value is sorted.
  for (;;) {
    ...
    if (iter->second->d->destruction_callback &&
        iter->second->d->destroy_if_owner_quits) {
      try {
      ...
      // if there is no destruction_callback, release object from clients
      // ownership (because the client hung up)
...
bool Container::load_plugin(const std::string &so_filename,
  ...
  if (!plugin_available) {
    d->plugins.push_back(
        std::make_pair(so_filename, std::move(std::unique_ptr<LibraryLoader>(
                                        new LibraryLoader(so_filename)))));
...
bool Container::unload_plugin(const std::string &so_filename,
                              const std::vector<std::string> &args) {
...
void Container::clear() { d->clear(); }

void Container::add_additional_info_callback(
    const std::string &name, std::function<std::string(void)> callback) {
...
SaftbusInfo Container::get_status() {
  ...
  for (auto &obj : d->objects) {
    ...
    object_info.signal_fds_use_count_and_dropped_signals =
        obj.second->d->signal_fds_use_count_and_dropped_signals;
    object_info.owner = obj.second->d->owner;
    object_info.has_destruction_callback =
        obj.second->d->destruction_callback ? true : false;
