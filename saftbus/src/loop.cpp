...
Source::Source() {
  if (id_counter == -1)
    ++id_counter; // prevent id_counter to produce an id of 0 (no source should
                  // have id 0)
  id = ++id_counter;
...
void Source::add_poll(pollfd *pfd) { pfds.push_back(pfd); }
void Source::remove_poll(pollfd *pfd) {
  pfds.erase(pfds.begin(), std::remove(pfds.begin(), pfds.end(), pfd));
}
void Source::clear_poll() { pfds.clear(); }
long Source::get_id() { return id; }
...
Loop::Loop() : d(new Impl) {
  ...
  if (d->id_counter == -1)
    ++d->id_counter; // prevent d->id_counter to produce an id of 0 (no source
                     // should have id 0)
  d->id = ++d->id_counter;
...
bool Loop::iteration(bool may_block) {
  ...
  for (auto &source : d->sources) {
    ...
    source->prepare(
        timeout_from_source); // source may leave timeout_from_source unchanged
    ...
    for (auto it = source->pfds.cbegin();
         it != source->pfds.cbegin() + source->pfds.size(); ++it) {
      // create a packed array of pfds that can be passed to poll()
      pfds.push_back(**it);
      // also create an array of pointers to pfds to where the poll() results
      // can be copied back
  ...
  for (auto &source : d->sources) {
    ...
    if (source->check()) {       // if check returns true, dispatch is called
      if (!source->dispatch()) { // if dispatch returns false, the source is
                                 // removed
        source.reset();
      }
    }
  ...
  if (d->running_depth == 1) {
    ...
    d->sources.erase(
        std::remove_if(d->sources.begin(), d->sources.end(),
                       [](std::unique_ptr<Source> &s) { return !s; }),
        d->sources.end());
...
bool Loop::quit() { return d->running = false; }
...
SourceHandle Loop::connect(std::unique_ptr<Source> source) {
  // std::cerr << "Loop::connect " << source->type() << "   size= " <<
  // d->sources.size() << std::endl;
  source->loop = this;
  SourceHandle result;
  result.loop_id = d->id;
...
void Loop::remove(SourceHandle s) {
  if (s.loop_id == d->id) { // make sure s was connected to this loop
    ...
    if ((source = std::find(source, d->added_sources.end(), s)) !=
        d->added_sources.end()) {
      source->reset();
    }
  }
...
TimeoutSource::TimeoutSource(std::function<bool(void)> s,
                             std::chrono::milliseconds i,
                             std::chrono::milliseconds o)
    : slot(s), interval(i),
      dispatch_time(std::chrono::steady_clock::now() + o) {
...
TimeoutSource::TimeoutSource(std::function<bool(void)> s,
                             std::chrono::milliseconds i)
    : slot(s), interval(i),
      dispatch_time(std::chrono::steady_clock::now() + i) {
  if (interval <= std::chrono::milliseconds(0)) {
    interval = std::chrono::milliseconds(1);
  }
}
...
IoSource::IoSource(std::function<bool(int, int)> s, int f, int c) : slot(s) {
  pfd.fd = f;
  pfd.events = c | POLLHUP | POLLERR;
  pfd.revents = 0;
  add_poll(&pfd);
}
IoSource::~IoSource() { remove_poll(&pfd); }
