...
/	The file descriptor integer value serves as a unique id to identify this
other process.
*/
struct Client {
  int socket_fd;    // the file descriptor is a unique number and is used as a
                    // client id
  ...
  Client(int fd, pid_t pid, SourceHandle h)
      : socket_fd(fd), process_id(pid), io_source(h) {}
  // LOGGING: add logging here
  ~Client() {
    Loop::get_default().remove(io_source);
    if (signal_fd_use_count.size() > 0) {
      // OLD_DEBUG: std::cerr << "not all signal fds of client " <<
      // (int)socket_fd << " were closed" << std::endl;
    }
    /* close socket_fd */
    close(socket_fd);
    /* close all signal_fds */
    for (auto &fd_use_count : signal_fd_use_count) {
      // OLD_DEBUG: std::cout << "close signal fd " << fd_use_count.first << "
      // with " << fd_use_count.second << " users " << std::endl;
  ...
  void use_signal_fd(int fd) {
    ...
    ++count;
    // OLD_DEBUG: std::cout << "Client::use_signal_fd(" << fd << ") count=" <<
    // count << std::endl;
  }
  void release_signal_fd(int fd) {
    ...
    if (count < 0) {
      std::cerr << "fatal error: " << __FILE__ << ":" << __LINE__
                << " signal use count < 0 must not happen" << std::endl;
      assert(false);
    }
    /*
    /	don't close even if use count reaches 0, because the client might create
    other proxies later /	if fd would be closed here, client must be able
    to detect this in the proxy constructor and / 	send a new
    socket-pair-fd so that the connection can be re-established / 	All
    signal file descriptors are closed when the client disconnects
    */
  }
};
...
struct ServerConnection::Impl {
  ...
  int calling_client_id; // this is equal to the client id as long as a client
                         // request is handled
  Impl(ServerConnection *connection)
      : container_of_services(connection), calling_client_id(-1) {}
  ~Impl() {}
...
bool ServerConnection::Impl::accept_client(int fd, int condition) {
  ...
  if (condition & POLLIN) {
    ...
    if (result != sizeof(pid)) {
      std::cerr << "Error in ServerConnection::Impl::accept_client: read "
                   "unexpected number of bytes"
                << std::endl;
    }
    auto handle = Loop::get_default().connect<IoSource>(
        std::bind(&ServerConnection::Impl::handle_client_request, this,
                  std::placeholders::_1, std::placeholders::_2),
        client_socket_fd, POLLIN | POLLHUP | POLLERR);
    /* register the client */
    clients.push_back(std::move(
        std::unique_ptr<Client>(new Client(client_socket_fd, pid, handle))));
    /* send the ID back to client (the file descriptor integer number is used as
     * ID) */
    result =
        write(client_socket_fd, &client_socket_fd, sizeof(client_socket_fd));
    if (result != sizeof(client_socket_fd)) {
      std::cerr << "Error in ServerConnection::Impl::accept_client: write "
                   "returns unexpected number of bytes"
                << std::endl;
    }
  }
  return true;
...
bool ServerConnection::Impl::handle_client_request(int fd, int condition) {
  ...
  */
  ...
  if (condition & (POLLIN | POLLHUP)) {
    ...
    /	we have to loop and read until all data is read and all remaining
    actions /	are executed. But not more then 100 times.
    */
    ...
    if (!container_of_services.call_service(saftbus_object_id, fd, received,
                                            send)) {
      ...
      */
      ...
      std::string what(
          "remote call failed because service object was not found");
      send.put(what);
    }
    if (!send.empty()) {
      send.write_to(fd);
...
void ServerConnection::Impl::client_hung_up(int client_fd) {
  ...
  /	the container hast to remove all services previously owned by this
  client
  */
  // LOGGING: add logging here
...
ServerConnection::ServerConnection(
  ...
  if (unlink_result != 0) {
    msg << "could not unlink socket file " << socketname << ": "
        << strerror(errno);
  }
  struct sockaddr_un listen_sockaddr_un = {.sun_family = AF_LOCAL};
  strcpy(listen_sockaddr_un.sun_path, socketname.c_str());
  int bind_result = bind(base_socket_fd, (struct sockaddr *)&listen_sockaddr_un,
                         sizeof(listen_sockaddr_un));
  if (bind_result != 0) {
    msg << "could not bind to socket: " << strerror(errno);
    throw std::runtime_error(msg.str());
  }
  chmod(socketname.c_str(),
        S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
  chmod(dirname.c_str(),
        S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);

  Loop::get_default().connect<IoSource>(
      std::bind(&ServerConnection::Impl::accept_client, d.get(),
                std::placeholders::_1, std::placeholders::_2),
      base_socket_fd, POLLIN | POLLHUP | POLLERR);
...
void ServerConnection::register_signal_id_for_client(int client_fd,
                                                     int signal_fd) {
...
void ServerConnection::unregister_signal_id_for_client(int client_fd,
                                                       int signal_fd) {
  auto client = std::find(d->clients.begin(), d->clients.end(), client_fd);
  // LOGGING: add logging here
  if (client == d->clients.end()) {
...
int ServerConnection::get_calling_client_id() { return d->calling_client_id; }
void set_owner();
void release_owner();
void owner_only();
