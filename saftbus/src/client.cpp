...
ClientConnection::ClientConnection(const std::string &socket_name)
  ...
  if (socketname[0] != '/') {
    msg << "saftbus socketname " << socketname
  ...
  int connect_result =
      connect(base_socket_fd, (struct sockaddr *)&call_sockaddr_un,
              sizeof(call_sockaddr_un));
  if (connect_result != 0) {
    msg << "cannot connect to socket: " << socketname
        << ". Possible reasons: server not running, wrong socket path (set "
           "SAFTBUS_SOCKET_PATH evironment variable), or wrong permissions";
    throw saftbus::Error(msg.str());
  }
  ...
  if (read(d->pfd.fd, &d->client_id, sizeof(d->client_id)) !=
      sizeof(d->client_id)) {
...
ClientConnection::~ClientConnection() { close(d->pfd.fd); }
...
int ClientConnection::atomic_send_and_receive(Serializer &serializer,
...
struct Proxy::Impl {
  ...
  int client_id, signal_group_id; // is determined at registration time and
                                  // needs to be saved for de-registration
  Serializer send;
  Deserializer received;
...
int SignalGroup::register_proxy(Proxy *proxy) {
  ...
  // std::cerr << "register_proxy: sending one fd " << d->fd_pair[0] << " for
  // service to send signals " << std::endl; std::cerr << "register_proxy:
  // keeping one fd " << d->fd_pair[1] << " for us to receive signals " <<
  // std::endl;
  ...
  if (d->signal_group_id == -1) {
    ...
    close(d->fd_pair[0]); // close the fd after sending it to the server
                          ...
                          // when the server closed the other end (because here
                          // we still have an open descriptor to the same end)
    if (fdresult <= 0) {
      throw saftbus::Error(
          "SignalGroup::register_proxy cannot send file descriptor to server");
    }
  }
...
void SignalGroup::unregister_proxy(Proxy *proxy) {
  ...
  d->proxies.erase(std::remove(d->proxies.begin(), d->proxies.end(), proxy),
                   d->proxies.end());
}

int SignalGroup::get_fd() { return d->pfd.fd; }

// Wait for singals to arrive. Don't wait more than timeout_ms milliseconds.
// If there are multiple signals waiting in the queue, they are all processed
// before the function returns. Return value:
...
int SignalGroup::wait_for_signal(int timeout_ms) {
  ...
  if (result >= 0) {
    ...
    while (wait_for_one_signal(0))
      ; // this loop will end if the timeout was hit (if no further signals are
        // present)
  }
  return result;
}

// Wait for a singal signal to arrive. Don't wait more than timeout_ms
// milliseconds. Return value:
//   > 0 if a signal was received
//     0 if timeout was hit
//   < 0 in case of failure (e.g. service object was destroyed)
int SignalGroup::wait_for_one_signal(int timeout_ms) {
  // std::cerr << "wait_for_one_signal(" << timeout_ms << ") on fd " <<
  // d->pfd.fd << std::endl;
  int result;
  {
      ...
      if (d->pfd.revents & (POLLIN | POLLHUP)) {
        ...
        d->received.get(signal_no);
        {
          ...
          for (auto &proxy : d->proxies) {
            // std::cerr << "proxy object id = " << proxy->d->saftbus_object_id
            // << "  signal_group_id = " << proxy->d->signal_group_id <<
            // std::endl;
...
Proxy::Proxy(const std::string &object_path, SignalGroup &signal_group,
  ...
  int interface_no =
      0; // Container_Service has only 1 interface with interface_no 0
  ...
  // if we get saftbus_object_id=-1, the object path was found found bu one of
  // the requested interfaces is not implemented
  if (d->saftbus_object_id == -1) {
    std::ostringstream msg;
    msg << "object \"" << object_path
        << "\" does not implement requested interfaces: ";
    for (auto &interface_name : interface_names) {
      if (d->interface_name2no_map.find(interface_name) ==
          d->interface_name2no_map.end()) {
...
Proxy::~Proxy() {
  ...
  d->send.put(1); // 1 is the special object id that adresses the
                  // ContainerService wich provides the unregister_proxy method
  int interface_no = 0;
...
bool Proxy::signal_dispatch(int interface_no, int signal_no,
                            Deserializer &signal_content) {
  return false;
}
SignalGroup &Proxy::get_signal_group() { return *d->signal_group; }
...
Serializer &Proxy::get_send() { return d->send; }
Deserializer &Proxy::get_received() { return d->received; }

int Proxy::get_saftbus_object_id() { return d->saftbus_object_id; }

rtpi::mutex &Proxy::get_client_socket_mutex() {
  return get_connection().d->connection_mutex;
}
rtpi::mutex &Proxy::get_proxy_mutex() { return d->proxy_mutex; }

int Proxy::interface_no_from_name(const std::string &interface_name) {
  std::map<std::string, int>::iterator it =
      d->interface_name2no_map.find(interface_name);
...
Container_Proxy::Container_Proxy(
    const std::string &object_path, saftbus::SignalGroup &signal_group,
    const std::vector<std::string> &interface_names)
    : saftbus::Proxy(object_path, signal_group, interface_names) {
  interface_no = saftbus::Proxy::interface_no_from_name("Container");
}
std::shared_ptr<Container_Proxy>
Container_Proxy::create(const std::string &object_path,
                        saftbus::SignalGroup &signal_group,
                        const std::vector<std::string> &interface_names) {
  return std::make_shared<Container_Proxy>(object_path, signal_group,
                                           gen_interface_names());
}
bool Container_Proxy::signal_dispatch(int interface_no, int signal_no,
...
bool Container_Proxy::unload_plugin(
