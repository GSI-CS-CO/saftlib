...
Dice::Dice() : auto_throwing_enabled(false), throw_timeout_source() {}

Dice::~Dice() { stopThrowing(); }
...
void Dice::throwPeriodically(int period_ms) {
  if (auto_throwing_enabled) {
    throw saftbus::Error(saftbus::Error::INVALID_ARGS,
                         "periodic throwing is already enabled");
  }
...
MyType Dice::passthrough(const MyType &val) { return val; }
int Dice::passthrough2(const int &val) { return val; }
