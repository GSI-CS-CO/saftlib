...
DiceCup::DiceCup(const std::string &obj_path, saftbus::Container *cont)
    : Dice(), container(cont), object_path(obj_path) {}
DiceCup::~DiceCup() {
  std::cerr << "DiceCup::~DiceCup" << std::endl;
  // If the DiceCup is destroyed, all Dices contained in it will be destroyed,
  // too. Make sure that the service objects of these Dices are also removed
...
void DiceCup::addDice6(const std::string &name) {
  ...
  if (container) {
    std::unique_ptr<Dice6_Service> service(new Dice6_Service(
        instance.get(), std::bind(&DiceCup::removeDice, this, dice_name)));
...
void DiceCup::addDice12(const std::string &name) {
  ...
  if (container) {
    std::unique_ptr<Dice12_Service> service(new Dice12_Service(
        instance.get(), std::bind(&DiceCup::removeDice, this, dice_name)));
    container->create_object(dice_name, std::move(service));
  }
  dices[dice_name] = std::move(instance);
