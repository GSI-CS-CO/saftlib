...
void usage(char *argv0) {
  ...
  std::cout << "usage: " << argv0
            << " [-s] [-r <object-path>] [-l <plugin.so> {plugin-args}] [-u "
               "<plugin.so>] [-h|--help]"
            << std::endl;
  std::cout << std::endl;
  std::cout
      << "  -s           print saftbus status, i.e. all available services,"
      << std::endl;
  std::cout << "               loaded plugins and connected clients."
            << std::endl;
  std::cout << std::endl;
  std::cout
      << "  -r           remove the service with <object-path> from saftbus"
      << std::endl;
  std::cout << std::endl;
  std::cout << "  -l           load a share object file as plugin and execute "
               "the create_services"
            << std::endl;
  std::cout << "               function of the plugin. If the plugin was "
               "loaded before, the"
            << std::endl;
  std::cout << "               create_services function is called again. "
               "plugin-args are "
            << std::endl;
  std::cout
      << "               passed as arguments to the create_services function."
      << std::endl;
  std::cout << "               The meaning of plugin-args depends on the "
               "plugin (see plugin documentation)."
            << std::endl;
  std::cout << std::endl;
  std::cout << "  -u           unload plugin. This should only be done when no "
               "services"
            << std::endl;
  std::cout << "               that were created from code within that plugin "
               "are active."
            << std::endl;
  std::cout << std::endl;
...
void print_status(saftbus::SaftbusInfo &saftbus_info) {
  ...
  for (auto &object : saftbus_info.object_infos) {
    max_object_path_length =
        std::max(max_object_path_length, object.object_path.size());
  }
  std::cout << "services:" << std::endl;
  std::cout << "  " << std::setw(max_object_path_length) << std::left
            << "object-path"
            << " ID [owner] sig-fd/use-count/dropped-signals interface-names"
            << std::endl;
  std::vector<std::string> services;
  for (auto &object : saftbus_info.object_infos) {
    std::ostringstream line;
    line << "  " << std::setw(max_object_path_length) << std::left
         << object.object_path << " " << object.object_id << " ["
         << object.owner << "]";
    if (object.has_destruction_callback && object.destroy_if_owner_quits)
      line << "D ";
    else if (object.has_destruction_callback && !object.destroy_if_owner_quits)
      line << "d ";
    else
      line << "  ";

    for (auto &fd_user_dropped :
         object.signal_fds_use_count_and_dropped_signals) {
      line << fd_user_dropped.first << "/" << fd_user_dropped.second.first
           << "/" << fd_user_dropped.second.second << " ";
    }
  ...
  for (auto &client : saftbus_info.client_infos) {
    std::cout << "  " << client.client_fd << " (pid=" << client.process_id
              << ")" << std::endl;
  }
  ...
  try {
    ...
    if (argc > 1) {
      for (int i = 1; i < argc; ++i) {
        ...
        if (argvi == "-s") {
          saftbus::SaftbusInfo saftbus_info =
              saftbus::Container_Proxy::create()->get_status();
        ...
        if (argvi == "-l") {
          if ((++i) < argc) {
            ...
            saftbus::Container_Proxy::create()->load_plugin(so_filename,
                                                            plugin_args);
            return 1;
          } else {
          ...
          if ((++i) < argc) {
            ...
            saftbus::Container_Proxy::create()->unload_plugin(so_filename,
                                                              plugin_args);
            return 1;
          } else {
