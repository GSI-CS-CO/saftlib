...
void Socket::wait_for_client()
{
  ...
  if (_debug_level) std::cerr << "bind to file " << _filename << std::endl;
  int bind_result = bind (_create_socket, (struct sockaddr *)&_address, sizeof(_address) );
  if (bind_result != 0) {
    if (_debug_level) std::cerr << "port busy" << std::endl;
  ...
  {
    if (_debug_level) std::cerr << "wait_for_client exception: " << e.what() << std::endl;
    throw;
  }
  ...
  if (_debug_level) std::cerr << "wait_for_client dt = " << dt << " us" << std::endl;          
...
bool Socket::accept_connection(Glib::IOCondition condition)
{
  ...
  if (_new_socket > 0)
  {
    if (_debug_level) std::cerr << "client connected " << std::endl;
  }
