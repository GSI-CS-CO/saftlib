...
ChunckAllocatorRT::ChunckAllocatorRT(size_t max_chuncks, size_t chuncksize)
    : MAX_CHUNCKS(max_chuncks), CHUNCKSIZE(chuncksize),
      chuncks(reinterpret_cast<char *>(::malloc(max_chuncks * chuncksize))),
      backindices(
          reinterpret_cast<size_t *>(::malloc(max_chuncks * sizeof(size_t)))),
      indices(
          reinterpret_cast<size_t *>(::malloc(max_chuncks * sizeof(size_t)))),
      allocated_chuncks(0) {
...
void ChunckAllocatorRT::print_size() {
  std::cerr << "filled: " << allocated_chuncks << "/" << MAX_CHUNCKS
            << std::endl;
}
...
bool ChunckAllocatorRT::contains(char *ptr) {
  return (ptr >= &chuncks[0] && ptr < &chuncks[MAX_CHUNCKS * CHUNCKSIZE]);
}
bool ChunckAllocatorRT::full() { return allocated_chuncks == MAX_CHUNCKS; }
bool ChunckAllocatorRT::fits(size_t n) { return n <= CHUNCKSIZE; }
...
Allocator::Allocator() {
  ...
  allocators = reinterpret_cast<ChunckAllocatorRT **>(
      ::malloc(num_allocators * sizeof(ChunckAllocatorRT *)));
  for (size_t i = 0; i < num_allocators; ++i) {
    int max_chuncks = 0;
    int chuncksize = 0;
    sscanf(ptr, "%d.%d", &max_chuncks, &chuncksize);
    if (i > 0) {
      assert(static_cast<int>(allocators[i - 1]->MAX_CHUNCKS) >
             max_chuncks); // later allocators must have fewer chuncks
      assert(static_cast<int>(allocators[i - 1]->CHUNCKSIZE) <
             chuncksize); // later allocators must use larger chuncks
    }
    printf("creating allocator with % 6d chuncks of size % 6d\n", max_chuncks,
           chuncksize);
    allocators[i] = new (::malloc(sizeof(ChunckAllocatorRT)))
        ChunckAllocatorRT(max_chuncks, chuncksize);
    while (*ptr)
      if (*ptr++ == ' ')
        break; // advance to next pair of numbers
  }
}
...
std::string Allocator::fillstate() {
  ...
  for (size_t i = 0; i < num_allocators; ++i) {
    msg << std::setw(9) << allocators[i]->CHUNCKSIZE << "   "
        << allocators[i]->allocated_chuncks << "/" << allocators[i]->MAX_CHUNCKS
        << std::endl;
  }
  msg << std::setw(9) << "heap" << "   " << heap_allocations << "/-"
      << std::endl;
  return msg.str();
}
...
std::string print_fillstate() { return saftbus::get_allocator()->fillstate(); }
