...
static void help( void )
{
  ...
  std::cout
      ...
      << std::endl;
  std::cout
      << "  -X                   used with command 'inject': use development mode to inject without requiring a WhiteRabbit lock"
      << std::endl;
  ...
  std::cout << "  -t                   display the current temperature in Celsius (if sensor is available) "
            << std::endl;
  ...
  std::cout
      << "                                   snoop for <seconds> or use CTRL+C to exit (try 'snoop 0x0 0x0 0' for ALL)"
      << std::endl;
  std::cout << std::endl;
  std::cout << "  attach <path> [<poll-iv>]        instruct saftd to control a new device. " << std::endl;
  std::cout
      << "                                   <poll-iv> is the polling interval for MSI on USB devices (default is 1 ms)."
      << std::endl;
...
static std::string signals_dropped_by_saftlib( saftbus::SaftbusInfo& saftbus_info, const std::string& object_path )
{
  ...
  {
    if ( object.object_path == object_path )
    { // object path matches
                           ...
                           std::shared_ptr<SoftwareActionSink_Proxy> sink )
{
  ...
  if ( wrLocked )
  {
    ...
    if ( absoluteTime )
      std::cout << "WR locked, time: " << tr_formatDate( wrTime, UTC ? PMODE_UTC : PMODE_NONE, printJSON ) << std::endl;
    else
      std::cout << "WR locked, time: " << tr_formatDate( wrTime, pmode, printJSON ) << std::endl;
  }
  else
    std::cout << "no WR lock!!!" << std::endl;

  // display ECA status
  nFreeConditions = receiver->getFree();
  std::cout << "receiver free conditions: " << nFreeConditions;

  std::cout << ", max (capacity of HW): " << sink->getMostFull() << "(" << sink->getCapacity() << ")"
            << ", early threshold: " << sink->getEarlyThreshold() << " ns"
            << ", latency: " << sink->getLatency() << " ns" << std::endl;
  ...
  if ( allSinks.size() > 0 )
  {
    ...
    for ( i = allSinks.begin(); i != allSinks.end(); i++ )
    {
      ...
      std::cout << "  " << i->second << " (minOffset: " << aSink->getMinOffset() << " ns"
                << ", maxOffset: " << aSink->getMaxOffset() << " ns)" << std::endl;
      std::cout << "  -- actions: " << aSink->getActionCount() << ", delayed: " << aSink->getDelayedCount()
                << ", conflict: " << aSink->getConflictCount() << ", late: " << aSink->getLateCount()
                << ", early: " << aSink->getEarlyCount() << ", overflow: " << aSink->getOverflowCount()
      ...
      for ( j = allConditions.begin(); j != allConditions.end(); j++ )
      {
        ...
        if ( condition->getAcceptDelayed() )
          acceptFlagsConfigStr[0] = 'd';
        if ( condition->getAcceptConflict() )
          acceptFlagsConfigStr[1] = 'c';
        if ( condition->getAcceptEarly() )
          acceptFlagsConfigStr[2] = 'e';
        if ( condition->getAcceptLate() )
          acceptFlagsConfigStr[3] = 'l';
        std::cout << "  ---- "
                  << tr_formatActionEvent(
                         condition->getID(),
                         pmode,
                         printJSON ) // ID: "   << fmt << std::setw(width) << std::setfill('0') << condition->getID()
                  << ", mask: " << fmt << std::setw( width ) << std::setfill( '0' ) << condition->getMask()
                  << ", offset: " << fmt << std::setw( 9 ) << std::setfill( '0' ) << condition->getOffset()
                  << ", accept: " << acceptFlagsConfigStr << ", active: " << std::dec << condition->getActive()
                  << ", destructible: " << condition->getDestructible() << ", owner: " << condition->getOwner()
                  << ", dropped by saftlib: " << signals_dropped_by_saftlib( saftbus_info, *j ) << std::endl;
      } // for all conditions
    } // for all sinks
  } // display all sinks
} // displayStatus
...
int main( int argc, char** argv )
{
  ...
  uint64_t eventTNext =
      0x0; // time for next event (this value is added to the current time or the next PPS, see option -p
  ...
  if ( optind + 1 < argc )
  {
    ...
    else if ( strcasecmp( command, "snoop" ) == 0 )
    {
      ...
      {
        std::cerr << program << ": expecting three or four arguments: snoop <eventID> <mask> <offset> [<seconds>]"
                  << std::endl;
        return 1;
      }
    ...
    else if ( strcasecmp( command, "quit" ) == 0 )
    {
      ...
      std::cerr << program << ": unknown command: " << command << std::endl;
  } // commands
  ...
  try
  {
    ...
    std::shared_ptr<SoftwareActionSink_Proxy> sink =
        SoftwareActionSink_Proxy::create( receiver->NewSoftwareActionSink( "" ) );
    ...
    if ( eventInject )
    {
      ...
      if ( pmode & PMODE_HEX )
      {
        std::cout << "Injected event (eventID/parameter/time): 0x" << std::hex << std::setw( 16 ) << std::setfill( '0' )
                  << eventID << " 0x" << std::setw( 16 ) << std::setfill( '0' ) << eventParam << " 0x"
                  << std::setw( 16 ) << std::setfill( '0' ) << ( UTC ? eventTime.getUTC() : eventTime.getTAI() )
                  << std::dec << std::endl;
      }
