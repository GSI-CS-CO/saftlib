...
/* Prototypes */
/* ==================================================================================================== */
void        onAction( uint64_t      event,
                      uint64_t      param,
                      saftlib::Time deadline,
                      saftlib::Time executed,
                      uint16_t      flags,
                      int           rule );
...
void onAction( uint64_t      event,
               ...
               int           rule )
{
  std::cout << "Got event at: 0x" << std::hex << ( UTC ? executed.getUTC() : executed.getTAI() ) << " -> "
            << tr_formatDate(
                   executed, ( verbose_mode ? PMODE_VERBOSE : PMODE_NONE ) | ( UTC ? PMODE_UTC : PMODE_NONE ), false )
            << std::endl;
  if ( verbose_mode )
  {
    std::cout << "  Deadline (Diff.): 0x" << ( UTC ? deadline.getUTC() : deadline.getTAI() ) << " (0x"
              << ( deadline - executed ) << ")" << std::endl;
...
void onConflictCount( uint64_t count )
{
  conflict_counter = count;
}
void onDelayedCount( uint64_t count )
{
  delayed_counter = count;
}
...
int main( int argc, char** argv )
{
  ...
  while ( ( opt = getopt( argc, argv, ":seiUvht:" ) ) != -1 )
  {
    ...
    {
      setup_scu_bus = true;
      scu_bus_tag   = strtoul( optarg, NULL, 0 );
      break;
    }
    default:
    {
    ...
    try
    {
      ...
      if ( wrLocked )
      {
        ...
        if ( verbose_mode )
        {
          ...
          std::cout << "Current time is "
                    << tr_formatDate( wrTime, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false ) << std::endl;
        }
      }
      else
      {
      ...
      if ( !just_inject )
      {
        for ( std::map<std::string, std::string>::iterator it = outs.begin(); it != outs.end(); ++it )
        {
          ...
          if ( verbose_mode )
          {
            std::cout << "Info: Found " << it->first << std::endl;
          }
          total_ios++;
          ...
          if ( setup_io )
          {
            if ( output_proxy->getOutputEnableAvailable() )
            {
              if ( verbose_mode )
              {
                std::cout << "Turning output enable on... " << std::endl;
              }
          ...
          if ( io_partner != "" )
          {
            if ( verbose_mode )
            {
              std::cout << "Found Partner Path: " << io_partner << std::endl;
            }
            if ( setup_io )
            {
              ...
              if ( input_proxy->getInputTerminationAvailable() )
              {
                if ( verbose_mode )
                {
                  std::cout << "Turning input termination off... " << std::endl;
                }
          ...
          /* Setup conditions */
          std::shared_ptr<OutputCondition_Proxy> condition_high = OutputCondition_Proxy::create(
              output_proxy->NewCondition( false, ECA_EVENT_ID, ECA_EVENT_MASK, 0, true ) );
          std::shared_ptr<OutputCondition_Proxy> condition_low = OutputCondition_Proxy::create(
              output_proxy->NewCondition( false, ECA_EVENT_ID, ECA_EVENT_MASK, 100000000, false ) );
      ...
      if ( setup_scu_bus )
      {
        ...
        /* Get connection */
        std::shared_ptr<SCUbusActionSink_Proxy> e_scubus =
            SCUbusActionSink_Proxy::create( e_scubusses.begin()->second );
        std::shared_ptr<SCUbusCondition_Proxy> scubus_condition;
        scubus_condition = SCUbusCondition_Proxy::create(
            e_scubus->NewCondition( false, ECA_EVENT_ID, ECA_EVENT_MASK, 0, scu_bus_tag ) );
      ...
      if ( !external_trigger )
      {
        ...
        while ( 1 )
        {
          ...
          if ( verbose_mode )
          {
            std::cout << "Time (base):   0x" << std::hex << ( UTC ? wrTime.getUTC() : wrTime.getTAI() ) << std::dec
                      << " -> " << tr_formatDate( wrTime, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false )
                      << std::endl;
          }
          ...
          if ( first_pps )
          {
            wrNext    = ( wrTime - nsec_fraction ) + 2000000000;
            first_pps = false;
          }
          else
          {
            wrNext = ( wrTime - nsec_fraction ) + 1000000000;
          }

          /* Print next pulse and inject event */
          std::cout << "Next pulse at: 0x" << std::hex << ( UTC ? wrNext.getUTC() : wrNext.getTAI() ) << std::dec
                    << " -> " << tr_formatDate( wrNext, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false )
                    << std::endl;
          ...
          while ( wrNext > receiver->CurrentTime() )
          {
            ...
            if ( verbose_mode )
            {
              ...
              std::cout << "Time (wait):   0x" << std::hex << ( UTC ? time.getUTC() : time.getTAI() ) << std::dec
      ...
      else
      {
        ...
        std::shared_ptr<SoftwareCondition_Proxy> condition =
            SoftwareCondition_Proxy::create( sink->NewCondition( false, ECA_EVENT_ID, ECA_EVENT_MASK, 0 ) );
