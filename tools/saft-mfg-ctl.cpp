...
static void timer_stop()
{
  ...
  std::cout << diff( time_start, time_end ).tv_sec << "." << std::setfill( '0' ) << std::setw( 9 )
            << diff( time_start, time_end ).tv_nsec << std::endl;
}
...
void test_master_fg( std::shared_ptr<SCUbusActionSink_Proxy> scu,
...
{
  if ( loglevel > 0 )
    std::cout << fg_name << " started at "
              << tr_formatDate( time, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false ) << std::endl;
}
static void on_master_armed( std::string fg_name, bool armed )
{
  if ( loglevel > 0 )
    std::cout << fg_name << ( armed ? " armed" : " disarmed" ) << std::endl;
}
static void on_master_enabled( std::string fg_name, bool enabled )
{
  if ( loglevel > 0 )
    std::cout << fg_name << ( enabled ? " enabled" : " disabled" ) << std::endl;
}
static void on_master_running( std::string fg_name, bool running )
{
  if ( loglevel > 0 )
    std::cout << fg_name << ( running ? " running" : " not running" ) << std::endl;
}
static void on_master_refill( std::string fg_name )
{
  if ( loglevel > 0 )
    std::cout << fg_name << " refill request" << std::endl;
}
static void on_master_stop( std::string   fg_name,
                            saftlib::Time time,
                            bool          abort,
                            bool          hardwareMacroUnderflow,
                            bool          microControllerUnderflow )
{
  if ( loglevel > 0 )
    std::cout << fg_name << " stopped at "
              << tr_formatDate( time, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false ) << std::endl;
  // was there an error?
  if ( abort )
    std::cerr << "Fatal error: Abort was called!" << std::endl;
...
{
  std::cout << "All fgs stopped at " << tr_formatDate( time, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false )
            << std::endl;
  {
...
static void* startFg( void* arg )
{
  ...
  if ( loglevel > 1 )
    std::cout << "startFg Loop ended" << std::endl;
...
{
  std::cerr << "warning: no input data received via stdin within 2s, run with '-h' for help. continuing to wait ..."
            << std::endl;
}
                     ...
                     uint32_t                                tag )
{
  ...
  std::shared_ptr<MasterFunctionGenerator_Proxy> master_gen =
      MasterFunctionGenerator_Proxy::create( master_fgs.begin()->second );
  ...
  {
    if ( loglevel > 1 )
      std::cout << "Created thread for receiving signals " << std::endl;
  }
  ...
  // for (; names.size()>1; names.pop_back())
  {
    ...
    if ( sync_arm )
    {
      ...
      if ( loglevel > 0 )
        std::cout << "Loading and arming (sync) FG sets via Master " << coeff_a.size() << " * " << coeff_a[0].size()
                  << std::endl;
      if ( loglevel > 1 )
        timer_start();
      master_gen->AppendParameterSets( coeff_a, coeff_b, coeff_c, step, freq, shift_a, shift_b );
      if ( loglevel > 1 )
        timer_stop();
      if ( loglevel > 0 )
      {
    ...
    {
      if ( loglevel > 0 )
        std::cout << "Loading and arming (async) FG sets via Master " << coeff_a.size() << " * " << coeff_a[0].size()
                  << std::endl;
      {
        std::lock_guard<std::mutex> lock( fg_all_armed_mutex );
        fg_all_armed = false;
      ...
      }
      {
        ...
        if ( !fg_all_armed )
        {
          all_armed_cond_var.wait( lock,
                                   []()
                                   {
                                     return fg_all_armed == true;
                                   } );
        }
      }
    }
...

    {
      ...
      if ( !fg_all_stopped )
      {
        all_stopped_cond_var.wait( lock,
                                   []()
                                   {
                                     return fg_all_stopped == true;
                                   } );
        if ( names.size() == 2 )
          loopStopped.store( true );
      }
    }
    if ( loglevel > 0 )
    {
