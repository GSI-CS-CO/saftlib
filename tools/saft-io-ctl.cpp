...
static int  io_setup( int  io_oe,
                      ...
                      bool set_stime,
                      bool verbose_mode );
static int  io_create( bool     disown,
                       uint64_t eventID,
                       uint64_t eventMask,
                       int64_t  offset,
                       uint64_t flags,
...
static void
io_catch_input( uint64_t event, uint64_t param, saftlib::Time deadline, saftlib::Time executed, uint16_t flags );
                      ...
                      bool     translate_mask )
{
  ...
  try
  {
    ...
    if ( flags & ( 1 << ECA_EARLY ) )
    {
      io_AcceptEarly = true;
    }
    if ( flags & ( 1 << ECA_CONFLICT ) )
    {
      io_AcceptConflict = true;
    }
    if ( flags & ( 1 << ECA_DELAYED ) )
    {
      io_AcceptDelayed = true;
    }
    ...
    if ( translate_mask )
    {
      condition = OutputCondition_Proxy::create(
          output_proxy->NewCondition( true, eventID, tr_mask( eventMask ), io_offset, io_edge ) );
    }
    else
    {
      condition =
          OutputCondition_Proxy::create( output_proxy->NewCondition( true, eventID, eventMask, io_offset, io_edge ) );
    }
  ...
  try
  {
    ...
    for ( std::map<std::string, std::string>::iterator it = outs.begin(); it != outs.end(); ++it )
    {
      ...
      for ( unsigned int condition_it = 0; condition_it < all_conditions.size(); condition_it++ )
      {
        ...
        {
          std::shared_ptr<OutputCondition_Proxy> destroy_condition =
              OutputCondition_Proxy::create( all_conditions[condition_it] );
          ...
          if ( destroy_condition->getDestructible() && ( destroy_condition->getOwner() == "" ) )
          {
            ...
            if ( verbose_mode )
            {
              std::cout << "Destroyed " << c_name << "!" << std::endl;
            }
          }
          else
          {
            if ( verbose_mode )
            {
              std::cout << "Found " << c_name << " but is not destructible!" << std::endl;
            }
          }
        }
      }
  ...
  try
  {
    ...
    for ( std::map<std::string, std::string>::iterator it = outs.begin(); it != outs.end(); ++it )
    {
      ...
      for ( unsigned int condition_it = 0; condition_it < all_conditions.size(); condition_it++ )
      {
        if ( ( ( ioNameGiven && ( it->first == ioName ) ) || !ioNameGiven ) )
        {
          std::shared_ptr<OutputCondition_Proxy> flip_condition =
              OutputCondition_Proxy::create( all_conditions[condition_it] );
          c_name = all_conditions[condition_it];
          ...
          else
          {
            if ( verbose_mode )
            {
              std::cout << "Found " << c_name << " but is already owned!" << std::endl;
            }
          }
        }
      }
    ...
    for ( std::map<std::string, std::string>::iterator it = outs.begin(); it != outs.end(); ++it )
    {
      ...
      for ( unsigned int condition_it = 0; condition_it < all_conditions.size(); condition_it++ )
      {
        if ( ( ( ioNameGiven && ( it->first == ioName ) ) || !ioNameGiven ) )
        {
          ...
          std::shared_ptr<OutputCondition_Proxy> info_condition =
              OutputCondition_Proxy::create( all_conditions[condition_it] );
          c_name                      = all_conditions[condition_it];
          ...
          if ( info_condition->getAcceptDelayed() )
          {
            std::cout << "d";
            flags = flags | ( 1 << ECA_DELAYED );
          }
          else
          {
            std::cout << ".";
          }
          if ( info_condition->getAcceptConflict() )
          {
            std::cout << "c";
            flags = flags | ( 1 << ECA_CONFLICT );
          }
          else
          {
            std::cout << ".";
          }
          if ( info_condition->getAcceptEarly() )
          {
            std::cout << "e";
            flags = flags | ( 1 << ECA_EARLY );
          }
          else
          {
            std::cout << ".";
          }
          if ( info_condition->getAcceptLate() )
          {
            std::cout << "l";
            flags = flags | ( 1 << ECA_LATE );
          }
          else
          {
            std::cout << ".";
...
static void io_help( void )
{
  ...
  std::cout
      << "  -n <name>:                                     Specify IO name (or leave out this option to see all IOs/conditions, see -i/-l)"
      << std::endl;
  std::cout << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "  These flags can be used in combination (e.g. flag 0x3 will accept late and early events)"
            << std::endl;
  std::cout << std::endl;
...
io_catch_input( uint64_t event, uint64_t param, saftlib::Time deadline, saftlib::Time executed, uint16_t flags )
{
  ...
  for ( std::map<std::string, uint64_t>::iterator it = map_PrefixName.begin(); it != map_PrefixName.end(); ++it )
  {
    if ( event == it->second )
    {
      catched_io = it->first;
  ...
  std::cout << "0x" << std::hex << setw( 16 + 1 ) << ( UTC ? time.getUTC() : time.getTAI() ) << std::dec << " "
            << tr_formatDate( time, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false );
  std::cout << std::endl;
}
    ...
    for ( std::map<std::string, std::string>::iterator it = inputs.begin(); it != inputs.end(); ++it )
    {
      if ( ( ( ioNameGiven && ( it->first == ioName ) ) || !ioNameGiven ) )
      {
        ...
        if ( !mode )
        {
          ...
          {
            if ( !sink )
              sink = SoftwareActionSink_Proxy::create( receiver->NewSoftwareActionSink( "" ) );
            proxies.push_back(
                SoftwareCondition_Proxy::create( sink->NewCondition( false, prefix, -2, IO_CONDITION_OFFSET ) ) );
                     ...
                     bool verbose_mode )
{
  ...
  io_set = set_oe | set_term | set_spec_out | set_spec_in | set_mux | set_pps | set_drive | set_stime | set_gate_out |
           set_gate_in;
  ...
  if ( verbose_mode )
  {
    if ( io_set )
    {
      std::cout << "Checking configuration feasibility..." << std::endl;
    }
    else
    {
      std::cout << "Checking current configuration..." << std::endl;
    }
  }
  ...
  try
  {
    ...
    else
    {
      if ( verbose_mode )
      {
        std::cout << "Info: Found " << ioName << " (" << io_type << ")" << "!" << std::endl;
      }
    }
    ...
    if ( io_set )
    {
      ...
      if ( set_oe )
      {
        output_proxy->setOutputEnable( io_oe );
      }
      if ( set_term )
      {
        input_proxy->setInputTermination( io_term );
      }
      if ( set_spec_out )
      {
        output_proxy->setSpecialPurposeOut( io_spec_out );
      }
      if ( set_spec_in )
      {
        input_proxy->setSpecialPurposeIn( io_spec_in );
      }
      if ( set_gate_out )
      {
        output_proxy->setGateOut( io_gate_out );
      }
      if ( set_gate_in )
      {
        input_proxy->setGateIn( io_gate_in );
      }
      if ( set_mux )
      {
        output_proxy->setBuTiSMultiplexer( io_mux );
      }
      if ( set_pps )
      {
        output_proxy->setPPSMultiplexer( io_pps );
      }
      if ( set_drive )
      {
        output_proxy->WriteOutput( io_drive );
      }
      if ( set_stime )
      {
        input_proxy->setStableTime( stime );
      }
    }
    else
    {
      ...
      if ( io_partner != "" )
      {
        std::cout << "  Partner: " << io_partner << std::endl;
      }
      std::cout << std::endl;
      ...
      if ( io_type != IO_CFG_FIELD_DIR_INPUT )
      {
        ...
        if ( output_proxy->ReadCombinedOutput() )
        {
          std::cout << "  CombinedOutput:   High" << std::endl;
        }
        else
        {
          std::cout << "  CombinedOutput:   Low" << std::endl;
        }

        /* Display output enable state */
        if ( output_proxy->getOutputEnableAvailable() )
        {
          if ( output_proxy->getOutputEnable() )
          {
            std::cout << "  OutputEnable:     On" << std::endl;
          }
          else
          {
            std::cout << "  OutputEnable:     Off" << std::endl;
          }
        }
        ...
        if ( output_proxy->getSpecialPurposeOutAvailable() )
        {
          if ( output_proxy->getSpecialPurposeOut() )
          {
            std::cout << "  SpecialOut:       On" << std::endl;
          }
          else
          {
            std::cout << "  SpecialOut:       Off" << std::endl;
          }
        }

        /* Display gate out state */
        if ( output_proxy->getGateOut() )
        {
          std::cout << "  GateOut:          On" << std::endl;
        }
        else
        {
          std::cout << "  GateOut:          Off" << std::endl;
        }

        /* Display BuTiS multiplexer state */
        if ( output_proxy->getBuTiSMultiplexer() )
        {
          std::cout << "  BuTiS t0 + TS:    On" << std::endl;
        }
        else
        {
          std::cout << "  BuTiS t0 + TS:    Off" << std::endl;
        }

        /* Display WR PPS multiplexer state */
        if ( output_proxy->getPPSMultiplexer() )
        {
          std::cout << "  WR PPS:           On" << std::endl;
        }
        else
        {
          std::cout << "  WR PPS:           Off" << std::endl;
        }
      ...
      if ( io_type != IO_CFG_FIELD_DIR_OUTPUT )
      {
        ...
        if ( input_proxy->getInputTerminationAvailable() )
        {
          if ( input_proxy->getInputTermination() )
          {
            std::cout << "  InputTermination: On" << std::endl;
          }
          else
          {
            std::cout << "  InputTermination: Off" << std::endl;
          }
        }
        ...
        if ( input_proxy->getSpecialPurposeInAvailable() )
        {
          if ( input_proxy->getSpecialPurposeIn() )
          {
            std::cout << "  SpecialIn:        On" << std::endl;
          }
          else
          {
            std::cout << "  SpecialIn:        Off" << std::endl;
          }
        }

        /* Display gate in state */
        if ( input_proxy->getGateIn() )
        {
          std::cout << "  GateIn:           On" << std::endl;
        }
        else
        {
          std::cout << "  GateIn:           Off" << std::endl;
        }
...
int main( int argc, char** argv )
{
  ...
  while ( ( opt = getopt( argc, argv, "a:j:n:o:t:q:e:m:p:d:k:swyb:rc:guxfzliUvh" ) ) != -1 )
  {
    ...
    case 'q':
    {
      io_spec_out  = atoi( optarg );
      set_spec_out = true;
      break;
    }
    case 'e':
    {
      io_spec_in  = atoi( optarg );
      set_spec_in = true;
      break;
    }
    case 'a':
    {
      io_gate_out  = atoi( optarg );
      set_gate_out = true;
      break;
    }
    case 'j':
    {
      io_gate_in  = atoi( optarg );
      set_gate_in = true;
      break;
    }
    case 'm':
    ...
    case 'b':
    {
      ...
      {
        prefix = strtoull( argv[optind - 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing prefix!" << std::endl;
        return ( __IO_RETURN_FAILURE );
      }
      break;
    }
    ...
    case 'c':
    {
      ...
      {
        std::cerr << "Error: Missing flags!" << std::endl;
        return ( __IO_RETURN_FAILURE );
      }
      ...
      {
        std::cerr << "Error: Missing level!" << std::endl;
        return ( __IO_RETURN_FAILURE );
      }
      break;
    }
  ...
  if ( ( ioc_create || ioc_disown ) && ioc_destroy )
  {
  ...
  if ( ios_snoop && ios_wipe )
  {
    std::cerr << "Incorrect arguments (disable input events or snoop)!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  else if ( ios_snoop && ios_setup_only )
  {
    std::cerr << "Incorrect arguments (enable input events or snoop)!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  else if ( ios_wipe && ios_setup_only )
  {
    std::cerr << "Incorrect arguments (disable or enable input events)!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  ...
  if ( optind + 1 == argc )
  {
    ...
    if ( ( io_oe || io_term || io_spec_out || io_spec_in || io_gate_out || io_gate_in || io_mux || io_pps || io_drive ||
           show_help || show_table || ios_snoop || ioc_create || ioc_disown || ioc_destroy || ioc_flip || ioc_list ||
           ioNameGiven || ioc_valid ) == false )
    {
      std::cerr
          << "Incorrect non-optional arguments (expecting at least the device name and one additional argument)(arg)!"
          << std::endl;
      return ( __IO_RETURN_FAILURE );
    }
  }
  ...
  {
    std::cerr
        << "Incorrect non-optional arguments (expecting at least the device name and one additional argument)(dev)!"
        << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  ...
  if ( verbose_mode )
  {
    std::cout << "Device " << deviceName << " found!" << std::endl;
  }

  /* Plausibility check */
  if ( io_oe > 1 || io_oe < 0 )
  {
    std::cout << "Error: Output enable setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_term > 1 || io_term < 0 )
  {
    std::cout << "Error: Input termination setting is invalid" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_spec_out > 1 || io_spec_out < 0 )
  {
    std::cout << "Error: Special (output) function setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_spec_in > 1 || io_spec_in < 0 )
  {
    std::cout << "Error: Special (input) function setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_gate_out > 1 || io_gate_out < 0 )
  {
    std::cout << "Error: Gate (output) setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_gate_in > 1 || io_gate_in < 0 )
  {
    std::cout << "Error: Gate (input) setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_mux > 1 || io_mux < 0 )
  {
    std::cout << "Error: BuTiS t0 gate/mux setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_pps > 1 || io_pps < 0 )
  {
    std::cout << "Error: WR PPS gate/mux setting is invalid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( io_drive > 1 || io_drive < 0 )
  {
    std::cout << "Error: Output value is not valid!" << std::endl;
    return ( __IO_RETURN_FAILURE );
  }
  if ( set_stime )
  {
    if ( stime % 8 != 0 )
    {
      std::cout << "Error: StableTime must be a multiple of 8 ns" << std::endl;
      return ( __IO_RETURN_FAILURE );
    }
    if ( stime < 16 )
    {
      std::cout << "Error: StableTime must be at least 16 ns" << std::endl;
      return ( __IO_RETURN_FAILURE );
    }
  }
  ...
  if ( ioNameGiven )
  {
    ...
    for ( std::map<std::string, std::string>::iterator it = outs.begin(); it != outs.end(); ++it )
    {
      ...
      if ( verbose_mode && ioNameExists )
      {
        std::cout << "IO " << ioName << " found (output)!" << std::endl;
        break;
      }
    }
    for ( std::map<std::string, std::string>::iterator it = ins.begin(); it != ins.end(); ++it )
    {
      ...
      if ( verbose_mode && ioNameExists )
      {
        std::cout << "IO " << ioName << " found (input)!" << std::endl;
        break;
      }
    }
  ...
  else
  {
    ...
    if ( show_table )
    {
      return_code = io_print_table( verbose_mode );
    }
    else if ( ios_wipe )
    {
      return_code = io_snoop( ios_wipe, ios_setup_only, ioc_dis_ios, prefix );
    }
    else if ( ios_snoop )
    {
      return_code = io_snoop( ios_wipe, ios_setup_only, ioc_dis_ios, prefix );
    }
    else if ( ios_setup_only )
    {
      return_code = io_snoop( ios_wipe, ios_setup_only, ioc_dis_ios, prefix );
    }
    else if ( ios_i_to_e )
    {
      return_code = io_list_i_to_e();
    }
    else if ( ioc_create )
    {
      return_code = io_create( ioc_disown, eventID, eventMask, offset, flags, level, negative, translate_mask );
    }
    else if ( ioc_destroy )
    {
      return_code = io_destroy( verbose_mode );
    }
    else if ( ioc_flip )
    {
      return_code = io_flip( verbose_mode );
    }
    else if ( ioc_list )
    {
      return_code = io_list();
