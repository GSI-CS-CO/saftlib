...
int main( int argc, char** argv )
{
  ...
  try
  {
    ...
    if ( create_sink )
    {
      ...
      if ( translate_mask )
      {
        condition = EmbeddedCPUCondition_Proxy::create(
            e_cpu->NewCondition( true, eventID, tr_mask( eventMask ), offset, tag ) );
      }
      else
      {
        condition = EmbeddedCPUCondition_Proxy::create( e_cpu->NewCondition( true, eventID, eventMask, offset, tag ) );
      }
    ...
    else if ( destroy_sink )
    {
      ...
      {
        std::shared_ptr<EmbeddedCPUCondition_Proxy> destroy_condition =
            EmbeddedCPUCondition_Proxy::create( all_conditions[condition_it] );
    ...
    else if ( list_conditions )
    {
      ...
      for ( unsigned int condition_it = 0; condition_it < all_conditions.size(); condition_it++ )
      {
        std::shared_ptr<EmbeddedCPUCondition_Proxy> info_condition =
            EmbeddedCPUCondition_Proxy::create( all_conditions[condition_it] );
        e_sink = all_conditions[condition_it];
