...
: Proxy(bus_type, name, object_path, interface_name, Glib::RefPtr&lt;Gio::DBus::InterfaceInfo&gt;(), flags)
{
}

Glib::RefPtr&lt;<xsl:value-of select="$iface"/>_Proxy&gt; <xsl:value-of select="$iface"/>_Proxy::create(
  const Glib::ustring&amp; object_path,
  const Glib::ustring&amp; name,
  Gio::DBus::BusType bus_type,
  Gio::DBus::ProxyFlags flags)
{
  return Glib::RefPtr&lt;<xsl:value-of select="$iface"/>_Proxy&gt;(new <xsl:value-of select="$iface"/>_Proxy(bus_type, name,
    object_path, "<xsl:value-of select="$iface_full"/>", flags));
}
