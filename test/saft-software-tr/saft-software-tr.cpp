/*  Copyright (C) 2020-2022 GSI Helmholtz Centre for Heavy Ion Research GmbH
 *
...
void EBslave::init()
{
  ...
  if ( _shutdown )
    return; // dont open the device if shutdown was initiated;
  ...
  *sel = 0xf;
  ...
  case EB_SLAVE_STATE_EB_CONFIG_FIRST:
    ...
    else if ( eb_rcount > 0 )
    {
      ...
      {
        wb_stbs.push_back(
            wb_stb( base_ret_adr, base_ret_adr, false, true ) ); // not a real strobe, just a pass-through
        wb_stbs.back().end_cyc = eb_flag_cyc;
        state                  = EB_SLAVE_STATE_EB_CONFIG_REST;
      }
    }
  ...
  case EB_SLAVE_STATE_EB_CONFIG_REST:
    ...
    else if ( eb_rcount > 0 )
    {
      ...
      if ( next_word( read_adr ) )
      {
        ...
        case 0x0:
          wb_stbs.push_back(
              wb_stb( error_shift_reg, error_shift_reg, false, true ) ); // not a real strobe, just a pass-through
          wb_stbs.back().end_cyc = eb_flag_cyc;
        ...
        case 0x34:
          wb_stbs.push_back(
              wb_stb( eb_msi_adr_first, eb_msi_adr_first, false, true ) ); // not a real strobe, just a pass-through
          wb_stbs.back().end_cyc = eb_flag_cyc;
          break;
        case 0x3c:
          wb_stbs.push_back(
              wb_stb( eb_msi_adr_last, eb_msi_adr_last, false, true ) ); // not a real strobe, just a pass-through
          wb_stbs.back().end_cyc = eb_flag_cyc;
          break;
        case 0x40: // msi_adr
...
void EBslave::msi_slave_out( std_logic_t* ack, std_logic_t* err, std_logic_t* rty, std_logic_t* stall, int* dat )
{
  ...
  *rty   = STD_LOGIC_0;
  *stall = STD_LOGIC_0;
  if ( msi_slave_out_ack )
    *ack = STD_LOGIC_1;
  if ( msi_slave_out_err )
    *err = STD_LOGIC_1;
  *dat = 0x0;
}
...
public:
  SDBrecords( const std::string& filename )
  {
    ...
    for ( int i = 0;; ++i )
    {
      ...
      // address of the SDB record, Content of the SDB record, Content of the SDB record puls the base address of that
      // record if it is a link to another SDB-record
...
struct SoftwareECA
{
  ...
  void inject()
  {
    ...
    for ( auto& walk : walker )
    {
      ...
      if ( verbosity >= 1 )
      {
        std::cout << "  " << std::dec << walk.first << " tag = " << walk.second.tag << "  next = " << walk.second.next
                  << std::endl;
      }
    }
    for ( auto& search : searches )
    {
      if ( verbosity >= 1 )
      {
        std::cout << "id = " << std::hex << id << " search.id = " << std::hex << search.id
                  << " search.mask = " << std::hex << search.mask << std::endl;
      }
      if ( ( id & search.mask ) == ( search.id & search.mask ) )
      {
        ...
        do
        {
          if ( verbosity >= 1 )
          {
            std::cout << "Condition matches walker " << std::dec << walker_idx << " with tag=" << std::dec
                      << walker[walker_idx].tag << std::endl;
          }
          ...
          if ( walker[walker_idx].fired == false )
          {
            Event                       new_event( id,
                             param,
                             offset_deadline,
                             walker[walker_idx].num,
                             walker[walker_idx].tag,
                             msi_target_adr,
                             msi_data | walker[walker_idx].num );
            std::lock_guard<std::mutex> lock( events_mutex );
            events.push_back( new_event );
            std::sort( events.begin(), events.end() );
            if ( verbosity >= 1 )
            {
              std::cout << "create event with id=" << std::hex << id << " param=" << param << " deadline=" << deadline
                        << std::dec << " num=" << walker_idx << " tag=" << walker[walker_idx].tag << std::endl;
  ...
  void push_search( const Search& s )
  {
    if ( std::find( searches.begin(), searches.end(), s ) == searches.end() )
    {
      ...
      if ( verbosity >= 1 )
      {
        std::cout << " pushed -> search: " << std::hex << searches.back().id << " " << searches.back().mask << " "
                  << std::dec << searches.back().first;
      }
    }
    else
    {
  ...
  void analyse_search_candidates_normal()
  {
    ...
    for ( unsigned i = 0; i < search_candidates.size(); ++i )
    {
      ...
      if ( verbosity >= 1 )
      {
        std::cout << std::dec << w1 << " " << std::hex << s1 << " => mask1=" << std::hex << std::setw( 16 )
                  << std::setfill( '0' ) << mask1 << " => mask2=" << std::hex << std::setw( 16 ) << std::setfill( '0' )
                  << mask2 << " is_prefix_mask(" << is_prefix_mask( mask1 ) << ") w1=" << w1 << " id1=" << id1
                  << " id_open=" << id_open;
      }
      ...
      if ( is_prefix_mask( mask1 ) && i )
      {
        if ( w0 == -1 )
          w0 = 0;
        if ( id0 )
          push_search( Search( id0, mask1, w0 ) );
        else if ( id1 )
          push_search( Search( id1, mask1, w0 ) );
      }
  ...
  {
    if ( search_candidates.size() /*&& search_candidates[0].first_walker == 0*/ &&
         search_candidates[0].search_event == 0 )
    {
    ...
    if ( verbosity >= 1 )
    {
      ...
      {
        std::cout << "   -> search: " << std::hex << searches[i].id << " " << searches[i].mask << " " << std::dec
                  << searches[i].first << std::endl;
      }
    }
  }
  ...
  struct Event
  {
    ...
    {
      return id == rhs.id && param == rhs.param && deadline == rhs.deadline /*&& num == rhs.num*/ && tag == rhs.tag &&
             msi_adr == rhs.msi_adr && msi_dat == rhs.msi_dat;
    }
    bool operator<( const Event& rhs ) const
    {
    ...
    for ( ; !FpgaReset::_reset_was_triggered; usleep( 100 ) )
    {
      for ( ;; )
      {
        ...
        if ( !software_eca->events.empty() && software_eca->events.front().deadline <= now )
        {
          if ( verbosity >= 1 )
          {
            std::cout << "creating action: now=" << std::dec << now
                      << " deadline=" << software_eca->events.front().deadline << std::endl;
          }
...
public:
  ...
  bool read_access( uint32_t adr, int sel, uint32_t* dat_out )
  {
    ...
    {
    case ECA_CHANNELS_GET:
      result = 2;
      break;
    case ECA_SEARCH_CAPACITY_GET:
      result = 0x200;
      break;
    case ECA_WALKER_CAPACITY_GET:
      result = 0x100;
      break;
    case ECA_LATENCY_GET:
      result = 12;
      break;
    case ECA_OFFSET_BITS_GET:
      result = 32;
      break;
    case ECA_CHANNEL_TYPE_GET:
      result = channel_type[_selected_channel];
      break;
    case ECA_CHANNEL_MAX_NUM_GET:
      result = channel_raw_max_num[_selected_channel];
      break;
    case ECA_CHANNEL_CAPACITY_GET:
      result = channel_raw_capacity[_selected_channel];
      break;
    case ECA_TIME_HI_GET:
      result = SoftwareECA::get_time_ns() >> 32;
      break;
    case ECA_TIME_LO_GET:
      result = SoftwareECA::get_time_ns() & 0xffffffff;
      break;
    case ECA_CHANNEL_MOSTFULL_ACK_GET:
      result = 0;
      break;
    case ECA_CHANNEL_MOSTFULL_CLEAR_GET:
      result = 0;
      break;
    case ECA_CHANNEL_VALID_COUNT_GET:
      result = 0;
      break;
    case ECA_CHANNEL_OVERFLOW_COUNT_GET:
      result = 0;
      break;
    case ECA_CHANNEL_FAILED_COUNT_GET:
      result = 0;
      break;
    default:
      return false;
  ...
  bool write_access( uint32_t adr, int sel, uint32_t dat )
  {
    ...
    case ECA_CHANNEL_MSI_SET_ENABLE_OWR:
      return true;
    ...
    case ECA_WALKER_WRITE_OWR:
      // std::cerr << "write walker tags " << std::hex << dat << std::endl; /*_walker_tags = _walker_tags_tmp;
      // _walker_tags_tmp.clear();*/
      if ( verbosity >= 1 )
      {
        std::cout << "walker " << std::dec << selected_walker << " next=" << software_eca.walker[selected_walker].next
                  << " tag=" << software_eca.walker[selected_walker].tag << " flags=" << std::hex
                  << software_eca.walker[selected_walker].flags << " channel=" << std::dec
                  << software_eca.walker[selected_walker].channel << " num=" << std::dec
                  << software_eca.walker[selected_walker].num << std::endl;
      }
      return true;
    }
    return false;
...
public:
  ...
  bool read_access( uint32_t adr, int sel, uint32_t* dat_out )
  {
    ...
    if ( !software_eca.actions.empty() )
    {
      ...
      switch ( adr - _adr_first )
      {
      case ECA_QUEUE_QUEUE_ID_GET:
        *dat_out = 0;
        return true;
      case ECA_QUEUE_FLAGS_GET:
        *dat_out = ( 1 << ECA_VALID );
        return true;
      case ECA_QUEUE_NUM_GET:
        *dat_out = software_eca.actions.front().num;
        return true;
      case ECA_QUEUE_EVENT_ID_HI_GET:
        *dat_out = software_eca.actions.front().id >> 32; /*eca_in_buffer[0]*/
        ;
        return true;
      case ECA_QUEUE_EVENT_ID_LO_GET:
        *dat_out = software_eca.actions.front().id & 0xffffffff; /*eca_in_buffer[1]*/
        ;
        return true;
      case ECA_QUEUE_PARAM_HI_GET:
        *dat_out = software_eca.actions.front().param >> 32; /*eca_in_buffer[2]*/
        ;
        return true;
      case ECA_QUEUE_PARAM_LO_GET:
        *dat_out = software_eca.actions.front().param & 0xffffffff; /*eca_in_buffer[3]*/
        ;
        return true;
      case ECA_QUEUE_TAG_GET:
        *dat_out = software_eca.actions.front().tag; /*eca_tag*/
        ;
        return true;
      case ECA_QUEUE_TEF_GET:
        *dat_out = 0; /*eca_in_buffer[5]*/
        ;
        return true;
      case ECA_QUEUE_DEADLINE_HI_GET:
        *dat_out = software_eca.actions.front().deadline >> 32; /*eca_in_buffer[6];*/
        return true;
      case ECA_QUEUE_DEADLINE_LO_GET:
        *dat_out = software_eca.actions.front().deadline & 0xffffffff; /*eca_in_buffer[7];*/
        return true;
      case ECA_QUEUE_EXECUTED_HI_GET:
        *dat_out = execution_time >> 32;
        return true;
      case ECA_QUEUE_EXECUTED_LO_GET:
        *dat_out = execution_time & 0xffffffff;
        return true;
      }
    }
    return false;
  ...
  bool write_access( uint32_t adr, int sel, uint32_t dat )
  {
    ...
    {
      if ( !software_eca.actions.empty() )
        software_eca.actions.pop_front();
      if ( verbosity >= 1 )
      {
...
public:
      ...
      , _instance( instance )
  {
    ...
    std::string build_id_text( "Project     : software timing receiver\n"
...
static void read_sdb( std::map<uint32_t, std::pair<uint32_t, uint32_t>>& result,
                      etherbone::Device                                  device,
                      eb_address_t                                       sdb_start_adr,
                      eb_address_t                                       base_adr = 0x0 )
{
...
int main( int argc, char* argv[] )
{
  ...
  try
  {
    std::cerr << "Loading SDB Record: " << DATADIR "/saftlib/software-tr.sdb" << std::endl;
    std::string sdb_filename( DATADIR "/saftlib/software-tr.sdb" );
    if ( argc != 1 )
    {
    ...
    for ( auto& device : sdb->create_devices<WatchdogMutex>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<BuildIdRom>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<WrPpsGenerator>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<EcaUnitControl>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<IoControl>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<EcaQueue>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<FpgaReset>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<EcaEventsIn>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<MsiMailbox>() )
      devices.push_back( device );
    for ( auto& device : sdb->create_devices<LM32ClusterInfoRom>() )
      devices.push_back( device );
    ...
    eb_slave                  = new EBslave( stop_until_connected, sdb->start_adr(), 0x20000, 0x2ffff );
    ...
    while ( FpgaReset::_reset_was_triggered == false || ending_countdown > 0 )
    {
      ...
      if ( cyc == STD_LOGIC_1 && stb == STD_LOGIC_1 )
      {
        ...
        if ( !found_adr )
        {
          if ( verbosity >= 0 )
          {
            std::cout << "address not mapped: 0x" << std::hex << std::setw( 8 ) << std::setfill( '0' ) << adr
                      << std::endl;
          }
          ack = STD_LOGIC_0;
          err = STD_LOGIC_1;
