...
static int test_attach_device( const std::string& device )
{
  ...
  try
  {
    ...
    if ( devices.find( logic_name ) != devices.end() )
    {
      ...
      try
      { // attach the same device again, this should produce an exception
        saftd->AttachDevice( logic_name, device );
...
static int test_remove_device()
{
  ...
  try
  {
    ...
    if ( devices.find( logic_name ) == devices.end() )
    {
      ...
      try
      { // remove the same device again, this should produce an exception
        saftd->RemoveDevice( logic_name );
...
static int test_inject_and_receive_event( const std::string& device )
{
  ...
  try
  {
    ...
    if ( par != event_param )
      throw std::runtime_error( "wrong event parameter" );
    if ( id != event_id )
      throw std::runtime_error( "wrong event id" );
...
static int test_software_condition_with_treads( const std::string& device )
{
  ...
  // this thread terminates the test after 30 seconds (if the success variable is not set to true at the end of the
  // test)
  std::thread timeout_thread( &timeout_thread_function, &success, __FUNCTION__, 90 );

  bool expect_signals = true;

  // start a thread that continually waits for signals until the *signals viable is set to false somewhere outside (to
  ...
  try
  {
    ...
    std::thread injector(
        []( bool* signals, std::shared_ptr<saftlib::TimingReceiver_Proxy> tr )
        {
          int i = 0;
          while ( *signals )
          {
    ...
    {
      conditions.push_back( std::make_shared<ConditionContainer>(
          saftlib::SoftwareCondition_Proxy::create( sas->NewCondition( true, 0, 0, 0 ) ) ) );
    }
    // sas->ToggleActive();
    ...
    if ( action_count == 0 )
    {
      std::cerr << "ERROR! on_action callback function was never called (" << signal_count << "signals were sent, "
                << N_conditions << " conditions were created)" << std::endl;
      ++number_of_failures;
    }
    else
    {
      std::cerr << "SUCCESS! received " << action_count << " actions (" << signal_count << " signals were sent, "
                << N_conditions << " conditions were created) " << std::endl;
    }
  }
  catch ( saftbus::Error& e )
...
int main( int argc, char** argv )
{
  ...
  for ( int i = 1; i < argc; ++i )
  {
    ...
    else if ( argvi == "-h" || argvi == "--help" )
    {
      ...
      std::cout
          << "  <eb-device-name> optional, if it is missing, saft-software-tr will be started and used for the tests."
          << std::endl;
      ...
      std::cout
          << "  saft-testbech                    run the testbench, the program will start an instance of saft-software-tr and "
          << std::endl;
      std::cout << "                                   saftbusd and attach the simulated hardware" << std::endl;
      std::cout
          << "  saft-testbech dev/ttyUSB0        run the testbench, the program will start an instance of and saftbusd and "
          << std::endl;
      std::cout << "                                   attach dev/ttyUSB0" << std::endl;
      std::cout
          << "  saft-testbech -s dev/ttyUSB0     run the testbench, an instance of saftbusd must already be running, the "
          << std::endl;
      std::cout << "                                   programm will attach dev/ttyUSB0" << std::endl;
      std::cout
          << "  saft-testbech -s                 run the testbench, an instance of saftbusd must already be running, the program"
          << std::endl;
      std::cout
          << "                                   will start an instance of saft-software-tr and will attach it to the running saftbusd"
          << std::endl;
      return 0;
    }
  ...
  if ( eb_device_name.size() == 0 )
  {
    ...
    if ( run_software_tr )
      (void)system( "killall -9 saft-software-tr" );
    if ( run_software_tr )
    {
      if ( msi_polling )
        system( "saft-software-tr -p &" );
      else
