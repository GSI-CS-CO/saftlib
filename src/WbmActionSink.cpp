...
void WbmActionSink::RecordMacro( uint32_t idx, const std::vector<WbmActionCmd>& commands )
{
  ...
  try
  {
    device.write( acwbm + SLAVE_REC_OWR, EB_DATA32, (eb_data_t)idx ); /* start recording */
  }
  catch ( etherbone::exception_t e )
  {
    throw saftbus::Error( saftbus::Error::INVALID_ARGS,
                          ( std::ostringstream()
                            << "Recording failed. Macro idx '" << idx
                            << "' is occupied or wbm-as out of macro memory. Choose a free index or delete macro(s)" )
                              .str() );
  }
