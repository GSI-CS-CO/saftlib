...
ActionSink::ActionSink( ECA&                eca_,
    ...
    , channel( channel_ )
    , num( num_ )
    , minOffset( -1000000000L )
    , maxOffset( 1000000000L )
    , signalRate( std::chrono::nanoseconds( 100000000L ) )
    , overflowCount( 0 )
    , actionCount( 0 )
    , lateCount( 0 )
    , earlyCount( 0 )
    ...
    , container( container_ )
{
  ...
  overflowUpdate = actionUpdate = lateUpdate = earlyUpdate = conflictUpdate = delayedUpdate =
...
void ActionSink::setMostFull( uint16_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set MostFull to 0" );
...
void ActionSink::setOverflowCount( uint64_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set OverflowCount to 0" );
  overflowCount = 0;
}
...
void ActionSink::setActionCount( uint64_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set ActionCount to 0" );
  actionCount = 0;
}
...
void ActionSink::setLateCount( uint64_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set LateCount to 0" );
  lateCount = 0;
}
...
void ActionSink::setEarlyCount( uint64_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set EarlyCount to 0" );
  earlyCount = 0;
}
...
void ActionSink::setConflictCount( uint64_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set ConflictCount to 0" );
  conflictCount = 0;
}
...
void ActionSink::setDelayedCount( uint64_t val )
{
  ownerOnly();
  if ( val != 0 )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "can only set DelayedCount to 0" );
  delayedCount = 0;
}
...
void ActionSink::receiveMSI( uint8_t code )
{
  ...
  case ECA_OVERFLOW:
    ...
    if ( exec > time )
      interval = std::chrono::duration_cast<std::chrono::milliseconds>( exec - time );
  ...
  case ECA_DELAYED:
    ...
    break;
  default:
    // LOGGING: add logging here - old: (clog << kLogErr << "Asked to handle an invalid MSI condition code in
    // ActionSink.cpp" << std::endl;)
    break;
  }
}
...
{
  // LOGGING: add logging here ("start",-1,-1);
  eb_data_t event_hi, event_lo, param_hi, param_lo, tag, tef, deadline_hi, deadline_lo, executed_hi, executed_lo,
      failed;
