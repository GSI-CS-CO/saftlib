...
static void on_action_op( uint64_t id, uint64_t param, saftlib::Time deadline, saftlib::Time executed, uint16_t flags )
{
  ...
  {
  // SIS, ESR, HEST
  case GTK7BC1L_TO_PLTKMH2__GS:
    gName = "(from TK)";
    break;
  case SIS18_RING:
    gName = "SIS18";
    break;
  case GTS1MU1_TO_GTS3MU1:
    gName = "NE3";
    break;
  case GTS3MU1_TO_GHFSMU1:
    gName = "NE4";
    break;
  case GTS1MU1_TO_GTE3MU1:
    gName = "NE5";
    break;
  case GHFSMU1_TO_GTS6MU1:
    gName = "NE5";
    break;
  case GTS6MU1_TO_ESR:
    gName = "NE12";
  ...
  case ESR_TO_GTV2MU2:
    gName = "NE8";
    break;
  case GTH4MU2_TO_GTV1MU1:
    gName = "NE8";
    break;
  case GHTBMU1_TO_YRT1MH2:
    gName = "NE11";
    break;
  // CRYRING
  case YRT1MH2_TO_CRYRING:
    gName = "NE11";
    break;
  case YRT1LQ1_TO_YRT1LC1:
    gName = "YRT1";
    break;
  case CRYRING_RING:
    gName = "CRYRING";
  ...
  case SEQ_START:
    ...
    {
      // old BPC finished
      if ( ( vaccAct == 0xffffffff ) && ( source == GTK7BC1L_TO_PLTKMH2__GS ) )
        std::cout << " [no vAcc requested] ";
      std::cout << std::endl;
  ...
  case COMMAND:
    if ( gid == idleSource )
    {
      ...
      {
      case 180:
        std::cout << std::endl << "   idle: " << gName;
        nCmd = 0;
        break; // idle since a long time ...
      default:
        if ( pmode & PMODE_VERBOSE )
          std::cout << std::endl << "(  idle: " << gName << ")";
        break;
...
static void help( void )
{
  ...
  std::cout << std::endl;
  std::cout << "  snoop <source>       snoops and displays Beam Production Chains, CTRL+C to exit (try 'snoop 0'')"
            << std::endl;
  std::cout << "                       machine: 0 UNILAC(TK), 1 SIS, 2 ESR, 3 CRYRING (injector), 4 CRYRING (NE11)"
            << std::endl;
  std::cout << std::endl;
...
int main( int argc, char** argv )
{
  ...
  if ( getVersion )
    std::cout << program << " version " << SAFT_LCD_VERSION << std::endl;
  ...
  if ( optind + 1 < argc )
  {
    ...
    if ( strcasecmp( command, "snoop" ) == 0 )
    {
      ...
      {
      case 0:
        source     = GTK7BC1L_TO_PLTKMH2__GS;
        idleSource = SIS18_RING;
        std::cout << "UNILAC(TK)";
        break;
      case 1:
        source     = SIS18_RING;
      ...
      case 2:
        ...
        break;
      case 3:
        source     = YRT1LQ1_TO_YRT1LC1;
        idleSource = CRYRING_RING;
        std::cout << "CRYRING injector";
        break;
      case 4:
        source     = YRT1MH2_TO_CRYRING;
        idleSource = CRYRING_RING;
        std::cout << "CRYRING cave";
        break;
      default:
        std::cerr << std::endl << program << ": invalid machine -- " << dummy << std::endl;
        return 1;
      } // switch dummy
      std::cout << std::endl << std::endl;
      if ( pmode & PMODE_VERBOSE )
        std::cout << "verbose mode: idle patterns and other BPCs are shown in brackets" << std::endl;
      std::cout << std::endl;
