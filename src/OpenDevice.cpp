...
bool OpenDevice::poll_msi( bool only_once )
{
  ...
  for ( int i = 0; i < MAX_MSIS_IN_ONE_GO; ++i )
  { // never more this many MSIs in one go
    cycle.open( device );
    ...
    if ( msi_cnt & 1 )
    {
      ...
      found_msi     = true;
      saftd->write(
          msi_adr, EB_DATA32, msi_dat ); // this functon is normally called by etherbone::Socket when it receives an MSI
    }
    if ( !( msi_cnt & 2 ) )
    { // no more msi to poll (second bit of msi_cnt is not set)
      break;
    }
  }
  if ( ( msi_cnt & 2 ) || found_msi )
  {
    ...
    poll_once = saftbus::Loop::get_default().connect<saftbus::TimeoutSource>(
        std::bind( &OpenDevice::poll_msi, this, only_once = true ),
...
OpenDevice::OpenDevice( const etherbone::Socket& socket, const std::string& eb_path, int polling_iv_ms, SAFTd* sd )
    ...
    , eb_forward_path( eb_path )
    , polling_interval_ms( polling_iv_ms )
    , saftd( sd )
    , check_msi_phase( true )
    , needs_polling( false )
{
  ...
  try
  {
    if ( saftd != nullptr )
    {
      ...
      check_irq =
          saftd->request_irq( *mbox, std::bind( &OpenDevice::check_msi_callback, this, std::placeholders::_1 ) );
      std::cerr << "msi_target_adr for poll check: " << std::hex << std::setw( 8 ) << std::setfill( '0' )
                << check_irq->address() << std::dec << std::endl;
