  ...
  try
  {
    ...
    for ( std::map<std::string, std::string>::iterator it = outs.begin(); it != outs.end(); ++it )
    {
      if ( ( ioNameGiven && ( it->first == ioName ) ) )
      {
        ...
        std::shared_ptr<Output_Proxy> output_proxy;
        output_proxy = Output_Proxy::create( it->second );
        if ( ( high_phase == 0.0 ) && ( low_phase == 0.0 ) && ( phase_offset == 0 ) )
        {
          ioClkStatus = output_proxy->StopClock();
        }
        else
        {
          ioClkStatus = output_proxy->StartClock( high_phase, low_phase, phase_offset );
        }
        if ( ioClkStatus )
        {
        ...
        {
          std::cout << "Warning: Output " << ioName
                    << " has been manually driven high while clock output is conceptually logically ORed!" << std::endl;
        }
      }
    }
  }
...
static void clk_gen_help( void )
{
  ...
  std::cout << "  -p <high phase[ns]> <low phase[ns]> <phase offset[ns]>: Start/Configure clock with the given phases"
            << std::endl;
  std::cout
  ...
  std::cout << "  -i:                                                     List all clock generator outputs"
            << std::endl;
  std::cout << "  -v:                                                     Switch to verbose mode" << std::endl;
  std::cout << "  -h:                                                     Print help (this message)" << std::endl;
  std::cout << std::endl;
  std::cout << "Note:" << std::endl;
  std::cout << "  Clock output is logically ORed with the manually driven output level (e.g. -d switch of saft-io-ctl)"
            << std::endl;
  std::cout << std::endl;
...
int main( int argc, char** argv )
{
  ...
  while ( ( opt = getopt( argc, argv, "n:p:f:sivh" ) ) != -1 )
  {
    ...
    case 'p':
    {
      ...
      {
        std::cerr << "Error: Missing value for high phase[ns]!" << std::endl;
        return ( -1 );
      }
      ...
      {
        std::cerr << "Error: Missing value for low phase[ns]!" << std::endl;
        return ( -1 );
      }
      if ( argv[optind + 1] != NULL )
      {
      ...
      {
        std::cerr << "Error: Missing value for phase offset[ns]!" << std::endl;
        return ( -1 );
      }
      break;
    }
    case 'f':
    {
      ...
      if ( argv[optind - 1] != NULL )
      {
        frequency = strtod( argv[optind - 1], &pEnd );
      }
      else
      {
        std::cerr << "Error: Missing value for frequency[Hz]!" << std::endl;
        return ( -1 );
      }
      if ( argv[optind - 0] != NULL )
      {
  ...
  {
    std::cerr << "Error: Start or stop the clock generator" << std::endl;
    return ( -1 );
  }
  if ( phase_config && frequency_config )
  {
    std::cerr << "Error: Choose phase(s) or frequency!" << std::endl;
    return ( -1 );
  }
  ...
  {
    return_code = clk_show_table();
  }
  else if ( start_clock )
  {
    return_code = clk_configure( high_phase, low_phase, phase_offset );
  }
  else if ( stop_clock )
  {
    return_code = clk_configure( 0.0, 0.0, 0 );
  }
  else
  {
    clk_gen_help();
