...
void MasterFunctionGenerator::on_fg_running( std::shared_ptr<FunctionGeneratorImpl>& fg, bool running )
{
  ...
  if ( generateIndividualSignals && std::find( activeFunctionGenerators.begin(), activeFunctionGenerators.end(), fg ) !=
                                        activeFunctionGenerators.end() )
  {
...
void MasterFunctionGenerator::on_fg_armed( std::shared_ptr<FunctionGeneratorImpl>& fg, bool armed )
{
  ...
  // std::cerr << "FG Armed  TID: " << syscall(SYS_gettid) << std::endl;
  if ( armed )
  {
    bool all_armed = true;
...
void MasterFunctionGenerator::on_fg_stopped( std::shared_ptr<FunctionGeneratorImpl>& fg,
                                             uint64_t                                time,
                                             bool                                    abort,
...
void MasterFunctionGenerator::InitializeSharedMemory( const std::string& shared_memory_name )
{
  ...
  {
    shm_params.reset(
        new boost::interprocess::managed_shared_memory( boost::interprocess::open_only, shared_memory_name.c_str() ) );
  }
...
void MasterFunctionGenerator::AppendParameterTuplesForBeamProcess( int beam_process )
{
  ...
  try
  {
...

    {
      ...
      typedef boost::interprocess::allocator<MapEntryType, boost::interprocess::managed_shared_memory::segment_manager>
                                                                                             MapAllocator;
      ...
      {
        /* Layout V0.1
        std::pair<ParameterVector *, std::size_t> fred = shm_params->find<ParameterVector>
        (activeFunctionGenerators[0]->GetName().c_str()); ParameterVector* paramVector = fred.first;
...
bool MasterFunctionGenerator::AppendParameterSets( const std::vector<std::vector<int16_t>>&       coeff_a,
                                                   ...
                                                   const std::vector<std::vector<unsigned char>>& shift_b )
{
  ...
  if ( coeff_b.size() != fgcount )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "coeff_b fgcount mismatch" );
  if ( coeff_c.size() != fgcount )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "coeff_c fgcount mismatch" );
  if ( step.size() != fgcount )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "step fgcount mismatch" );
  if ( freq.size() != fgcount )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "freq fgcount mismatch" );
  if ( shift_a.size() != fgcount )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "shift_a fgcount mismatch" );
  if ( shift_b.size() != fgcount )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "shift_b fgcount mismatch" );

  if ( fgcount > activeFunctionGenerators.size() )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "More datasets than function generators" );
  ...
  for ( std::size_t i = 0; i < fgcount; ++i )
  {
    if ( coeff_a[i].size() > 0 )
    {
      lowFill |= activeFunctionGenerators[i]->appendParameterSet(
          coeff_a[i], coeff_b[i], coeff_c[i], step[i], freq[i], shift_a[i], shift_b[i] );
    }
  }
...
void MasterFunctionGenerator::SetActiveFunctionGenerators( const std::vector<std::string>& names )
{
  ...
  for ( auto name : names )
  {
    if ( std::any_of( allFunctionGenerators.begin(),
                      allFunctionGenerators.end(),
                      [name]( std::shared_ptr<FunctionGeneratorImpl> fg )
                      {
                        return name == fg->GetName();
                      } ) == false )
    {
  ...
  for ( auto name : names )
  {
    auto fg_it = std::find_if( allFunctionGenerators.begin(),
                               allFunctionGenerators.end(),
                               [name]( std::shared_ptr<FunctionGeneratorImpl> fg )
                               {
                                 return name == fg->GetName();
...
{
  // DRIVER_LOG("",-1,-1);
  std::cerr << "MasterFG Timed out waiting" << std::endl;
