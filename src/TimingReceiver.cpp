...
TimingReceiver::TimingReceiver( SAFTd&              saftd,
                                const std::string&  n,
                                const std::string&  eb_path,
  ...
  for ( auto& io : ios )
  {
    ...
    if ( io.getDirection() == IO_CFG_FIELD_DIR_INPUT || io.getDirection() == IO_CFG_FIELD_DIR_INOUT )
    {
      std::unique_ptr<Input> input(
          new Input( *dynamic_cast<ECA_TLU*>( this ), input_path, output_path, io.getEcaIn(), &io, container ) );
      if ( container )
      {
        std::unique_ptr<Input_Service> service( new Input_Service( input.get() ) );
        input->set_service( service.get() );
    ...
    if ( io.getDirection() == IO_CFG_FIELD_DIR_OUTPUT || io.getDirection() == IO_CFG_FIELD_DIR_INOUT )
    {
      std::unique_ptr<Output> output(
          new Output( *dynamic_cast<ECA*>( this ), io, output_path, input_path, eca_channel_for_outputs, container ) );
      if ( container )
      {
        std::unique_ptr<Output_Service> service( new Output_Service( output.get() ) );
        output->set_service( service.get() );
...
saftlib::Time TimingReceiver::CurrentTime( bool devInject ) const
{
  if ( devInject )
  {
    std::cerr << "WARNING: Using development mode with a simulated WhiteRabbit lock. No proper timing possible!"
              << std::endl;
  }
