...
static void on_action( uint64_t id, uint64_t param, saftlib::Time deadline, saftlib::Time executed, uint16_t flags )
{
  ...
  if ( id == messageCounterID )
  {
    ...
    tmp   = ( param << 32 ) | ( ( param >> 32 ) & 0xffffffff );
    param = tmp;
    // <<<<<<< fix me (requires new GW for DM)
    ...
    if ( !firstRun )
    {
      ...
      {
        std::cout << std::dec << "msg total (lost: sum / network / overflow): " << messageCounterAll << " ("
                  << lostMessagesAll << " / " << lostMessagesAll - overflowCounterAll << " / " << overflowCounterAll
                  << "), msg diff (...): " << messageCounterDiff << " (" << lostMessagesDiff << " / "
                  << lostMessagesDiff - overflowCounterDiff << " / " << overflowCounterDiff << ")";
        if ( lostMessagesAll )
          std::cout << ": LOSS!";
        if ( lostMessagesDiff )
          std::cout << ": DIFFLOSS!";
  ...
  if ( pmode & PMODE_VERBOSE )
    std::cout << "MSG received: " << messageCounterAll << ", valid: " << (int)( !firstRun ) << std::hex << ", ID: 0x"
              << id << std::dec << std::endl;
...
static void help( void )
{
  ...
  std::cout << std::endl;
  std::cout << "  count                counts messages received from DM and compares with number of messages sent by DM"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "   with the number of messages actually sent ('set value') by the DM. This example will use the "
            << std::endl;
  std::cout << "   first timing receiver available in the host system and print only to screen in case of lost"
            << std::endl;
