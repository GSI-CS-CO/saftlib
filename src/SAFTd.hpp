...
/// The name "SAFTd" is kept for backwards compatibility with older saftlib versions, in order to keep the user facing
/// API stable. A better name would be saftlib::EbSocket, because it encapsulates an etherbone::Socket together with
/// some additional functions. SAFTd provides:
...
public:
  ...
  /// @param slot function object that is called when an MSI with the correct address (return value of this fuction)
  /// arrives
  /// @return an unique_ptr<IRQ> that can be used to obtain the address to trigger the slot function. The irq is
  /// released in the
...
private:
  ...
  // and the object_path of the SAFTd_Service (normally "/de/gsi/saftlib") as prefix for the object_path of
  // TimingReceiver_Service objects
