...
typedef boost::interprocess::allocator<ParameterTuple, boost::interprocess::managed_shared_memory::segment_manager>
                                                                    ShmemAllocator;
...
public:
  ...
  ///
  // @saftbus-export
  bool AppendParameterSets( const std::vector<std::vector<int16_t>>&       coeff_a,
...
protected:
  ...
  void on_fg_stopped( std::shared_ptr<FunctionGeneratorImpl>& fg,
                      uint64_t                                time,
                      bool                                    abort,
