...
static void
on_action_uni_cycle( uint64_t id, uint64_t param, saftlib::Time deadline, saftlib::Time executed, uint16_t flags )
{
  uint32_t gid;
  uint32_t evtNo;
  uint32_t vacc;
  ...
  if ( ( deadline - prevDeadline ) > 10000000 )
  { // new UNILAC cycle starts if diff > 10ms
    switch ( nCycle )
    {
    case 0: // print header
    ...
    default: // default
      ...
      std::cout << std::setw( 10 ) << nCycle << ":" << std::setw( 8 ) << pz1 << std::setw( 8 ) << pz2 << std::setw( 8 )
                << pz3 << std::setw( 8 ) << pz4 << std::setw( 8 ) << pz5 << std::setw( 8 ) << pz6 << std::setw( 8 )
                << pz7 << std::endl;
      break;
    } // switch nCycle
...
static void
on_action_uni_vacc( uint64_t id, uint64_t param, saftlib::Time deadline, saftlib::Time executed, uint16_t flags )
{
#define OBSCYCLES 200 // # of cycles observed before printing (a cycle takes 20ms)
  ...
  {
    for ( j = 0; j < NVACC; j++ )
    { // clear execution counter and flags
      for ( i = 0; i < NPZ; i++ )
        nExe[i][j] = 0;
      flagHighC[j]  = 0;
      flagRigid[j]  = 0;
  ...
  if ( ( deadline - prevDeadline ) > 10000000 )
  { // new UNILAC cycle starts if diff > 10ms
    if ( ( ( nCycle % OBSCYCLES ) == 0 ) && ( nCycle > 0 ) )
    { // observation period is over
      ...
      for ( j = 0; j < NVACC; j++ )
      {
        ...
        for ( i = 0; i < NPZ; i++ )
        {
          ...
          {
            std::cout << "    X";
            rate = 50.0 * (double)( nExe[i][j] ) /
                   (double)OBSCYCLES; // this assumes UNILAC runs at 50Hz which might not be true always
          } // if nExe
          else
            std::cout << "     ";
        } // for PZs
        if ( rate > 0.0001 )
          std::cout << std::setw( 6 ) << fixed << setprecision( 2 ) << rate; // printf was so easy :)
        std::cout << " ";
        if ( flagHighC[j] )
          std::cout << "H";
        if ( flagRigid[j] )
      ...
      std::cout << "." << std::flush; // user entertainment
...
static void help( void )
{
  ...
  std::cout
      ...
      << std::endl;
  std::cout << std::endl;
  std::cout << "  snoop  <type>        snoop events from WR-UNIPZ @ UNILAC,  <type> may be one of the following"
            << std::endl;
  std::cout << "            '0'        event display, but limited to GIDs of UNILAC and a subset of event numbers"
            << std::endl;
  std::cout << "            '1'        shows virtual accelerator executed at the seven PZs, similar to 'rsupcycle'"
            << std::endl;
  ...
  std::cout << "            '2'        shows virtual accelerator executed at the seven PZs, similar to 'eOverview'"
            << std::endl;
  std::cout << std::endl;
  std::cout << "This tool snoops and diplays UNILAC specific info for legacy timing groups." << std::endl;
  std::cout << std::endl;
  std::cout << "UNILAC is operated at 50Hz cycle rate. Legacy timing groups are named QR, QL, QN, HLI, HSI, AT and TK."
            << std::endl;
  ...
  std::cout << "The beams of the two injectors are accelerated into the 'Alvarez Section' (AT). The beam can be"
            << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "Shown are flags indicating special modes of operation: N(o beam), R(igid beam) and H(igh current beam)"
            << std::endl;
...
int main( int argc, char** argv )
{
  ...
  try
  {
    ...
    if ( uniSnoop )
    {
      ...
      // the following commented code might cause saftlib to hang in case actions are executed while conditions are
      // activated according to the developer, 'setActive' causes some heavy processing / book keeping in saftib.
      // Instead, it is suggested toggle the active-state of all actions in the 'sink'.
      /*
      for (int i=0; i<nPz; i++) {
