...
int EB_Source::add_fd( eb_user_data_t data, eb_descriptor_t fd, uint8_t mode )
{
  ...
  if ( ( mode & EB_DESCRIPTOR_IN ) != 0 )
    pfd.events |= POLLIN;
  if ( ( mode & EB_DESCRIPTOR_OUT ) != 0 )
    pfd.events |= POLLOUT;
...
int EB_Source::get_fd( eb_user_data_t data, eb_descriptor_t fd, uint8_t mode )
{
  ...
  // check if iterator is valid and (by chance or clever prediction) points to the correct element in the vector<pollfd>
  // fds
  ...
  ++self->fds_it; // expect the next call to this function will be with the fd of the proceeding element in
                  // vector<pollfd> fds
  return ( ( mode & EB_DESCRIPTOR_IN ) != 0 && ( flags & ( POLLIN | POLLERR | POLLHUP ) ) != 0 ) ||
