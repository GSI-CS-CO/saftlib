...
static bool fill( std::shared_ptr<FunctionGenerator_Proxy> gen, const ParamSet& params )
{
  return gen->AppendParameterSet(
      params.coeff_a, params.coeff_b, params.coeff_c, params.step, params.freq, params.shift_a, params.shift_b );
}
                     ...
                     bool*         continue_main_loop )
{
  *continue_main_loop = false;
  std::cout << "Function generator stopped at "
            << tr_formatDate( time, PMODE_VERBOSE | ( UTC ? PMODE_UTC : PMODE_NONE ), false ) << std::endl;
  // was there an error?
  if ( abort )
    ...
    if ( scan )
    {
      ...
      if ( nothing_owned( receiver ) )
      {
        try
        {
          ...
          if ( i_or_m == 'i' )
            scanning_result = fg_firmware->ScanFgChannels();
          if ( i_or_m == 'm' )
            scanning_result = fg_firmware->ScanMasterFg();
