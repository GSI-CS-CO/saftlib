...
/// @brief if the name and etherbone_path have '*' as last charater, the etherbone_path is scanned for matching device
/// files. All matching devices will be attached.
///
...
/// @param etherbone_path etherbone path. If name has a '*' as last character, etherbone_path needs '*' as last
/// character, too.
/// @param poll_interval_ms this is directly passed to AttachDevice function
void handle_wildcards_and_attach_device( saftlib::SAFTd*   saftd,
  ...
  if ( name.size() && name.back() == '*' )
  {
    if ( etherbone_path.size() && etherbone_path.back() != '*' )
    {
      throw saftbus::Error( saftbus::Error::INVALID_ARGS,
                            "if name has * wildcard as last char, etherbone_path also needs wildcard as last char" );
    }
    std::string path( "/" );
...
extern "C" void create_services( saftbus::Container* container, const std::vector<std::string>& args )
{
  ...
  container->create_object( saftd->getObjectPath(),
                            std::move( std::unique_ptr<saftlib::SAFTd_Service>(
                                new saftlib::SAFTd_Service( saftd.get(), std::bind( &destroy_service ), false ) ) ) );
  ...
  for ( auto& arg : args )
  {
    ...
    {
      if ( pos2 + 1 == path.size() )
      { // 2nd colon is there, but poll inteval is missing
        throw std::runtime_error( "expect <name>:<eb-path>[:<poll-interval>] as argument" );
      }
