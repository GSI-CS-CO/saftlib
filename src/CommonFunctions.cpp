...
std::string tr_formatActionEvent( uint64_t id, uint32_t pmode, bool json )
{
  ...
  if ( pmode & PMODE_HEX )
  {
    full << std::hex << std::setfill( '0' );
    width = 16;
    fmt   = "0x";
  }
  if ( pmode & PMODE_DEC )
  {
    full << std::dec << std::setfill( '0' );
    width = 20;
    fmt   = "0d";
  }
  ...
  if ( pmode & PMODE_VERBOSE )
  {
    full << " " << extra_mark << "FID" << extra_mark << ": " << fmt << std::setw( 1 ) << ( ( id >> 60 ) & 0xf )
         << extra_comma;
    full << " " << extra_mark << "GID" << extra_mark << ": " << fmt << std::setw( 4 ) << ( ( id >> 48 ) & 0xfff )
         << extra_comma;
    full << " " << extra_mark << "EVTNO" << extra_mark << ": " << fmt << std::setw( 4 ) << ( ( id >> 36 ) & 0xfff )
         << extra_comma;
    if ( fid == 0 )
    {
      // full << " FLAGS: N/A";
      full << " " << extra_mark << "FLAGS" << extra_mark << ": " << extra_mark << "N/A" << extra_mark << extra_comma;
      full << " " << extra_mark << "SID" << extra_mark << ": " << fmt << std::setw( 4 ) << ( ( id >> 24 ) & 0xfff )
           << extra_comma;
      full << " " << extra_mark << "BPID" << extra_mark << ": " << fmt << std::setw( 5 ) << ( ( id >> 10 ) & 0x3fff )
           << extra_comma;
      full << " " << extra_mark << "RES" << extra_mark << ": " << fmt << std::setw( 4 ) << ( id & 0x3ff )
           << extra_comma;
    } // if fid==0
    else if ( fid == 1 )
    {
      full << " " << extra_mark << "FLAGS" << extra_mark << ": " << fmt << std::setw( 1 ) << ( ( id >> 32 ) & 0xf )
           << extra_comma;
      full << " " << extra_mark << "BPC" << extra_mark << ": " << fmt << std::setw( 1 ) << ( ( id >> 34 ) & 0x1 )
           << extra_comma;
      full << " " << extra_mark << "SID" << extra_mark << ": " << fmt << std::setw( 4 ) << ( ( id >> 20 ) & 0xfff )
           << extra_comma;
      full << " " << extra_mark << "BPID" << extra_mark << ": " << fmt << std::setw( 5 ) << ( ( id >> 6 ) & 0x3fff )
           << extra_comma;
      full << " " << extra_mark << "RES" << extra_mark << ": " << fmt << std::setw( 4 ) << ( id & 0x3f ) << extra_comma;
    } // if fid==1
    else
    {
      full << " " << extra_mark << "Other" << extra_mark << ": " << fmt << std::setw( 9 ) << ( id & 0xfffffffff )
           << extra_comma;
    }
  }
  else
  {
    full << " " << extra_mark << "EvtID" << extra_mark << ": " << fmt << std::setw( width ) << std::setfill( '0' ) << id
         << extra_comma;
  }
...
std::string tr_formatActionParam( uint64_t param, uint32_t evtNo, uint32_t pmode, bool json )
{
  ...
  case 0x1:
    ...
    break;
  default:
    full << " " << extra_mark << "Param" << extra_mark << ": " << fmt << std::setw( width ) << param << extra_comma
         << extra_space;
  } // switch evtNo
...
std::string tr_formatActionFlags( uint16_t flags, uint64_t delay, uint32_t pmode, bool json )
{
  ...
  if ( json )
  {
    ...
    if ( flags & 4 )
    {
      full << "\"ConflictDelay_ns\": " << delay << ", ";
    }
    else
    {
      full << "\"ConflictDelay_ns\": 0, ";
    }
    if ( flags & 8 )
    {
      full << "\"Delayed_ns\": " << delay << ", ";
    }
    else
    {
      full << "\"Delayed_ns\": " << "0, ";
  ...
  else
  {
    if ( flags )
    {
      full << "!";
      if ( flags & 1 )
        full << "late (by " << delay << " ns)";
      if ( flags & 2 )
        full << "early (by " << -delay << " ns)";
      if ( flags & 4 )
        full << "conflict (delayed by " << delay << " ns)";
      if ( flags & 8 )
        full << "delayed (by " << delay << " ns)";
    } // if flags
  } // if json
