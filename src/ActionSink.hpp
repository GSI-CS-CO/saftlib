...
public:
  ...
  std::string NewConditionHelper( bool active, Args&&... args )
  {
    ...
    std::unique_ptr<ConditionType> condition(
        new ConditionType( this, number, active, std::forward<Args>( args )... ) );
    std::string path = condition->getObjectPath();
    if ( container )
    {
      std::unique_ptr<typename ConditionType::ServiceType> service( new typename ConditionType::ServiceType(
          condition.get(), std::bind( &ActionSink::removeCondition, this, condition.get() ) ) );
