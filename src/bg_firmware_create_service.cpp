...
extern "C" void create_services( saftbus::Container* container, const std::vector<std::string>& args )
{
  ...
  for ( unsigned i = 0; i < args.size(); ++i )
  {
    ...
    saftlib::TimingReceiver_Service* tr_service =
        dynamic_cast<saftlib::TimingReceiver_Service*>( container->get_object( device_object_path ) );
    ...
    if ( ( i + 1 ) < args.size() && all_devices.find( args[i + 1] ) == all_devices.end() )
    {
      ...
      {
        std::cerr << "Invalid cpu index " << cpu_idx << " , hardware has only " << tr->LM32Cluster::getCpuCount()
                  << " LM32 cores. " << std::endl;
        return;
      }
      if ( cpu_idx >= 0 )
    ...
    std::unique_ptr<saftlib::BurstGenerator_Service> service( new saftlib::BurstGenerator_Service(
        burstgenerator_fw.get(), std::bind( &saftlib::TimingReceiver::removeAddon, tr, addon_name ) ) );
