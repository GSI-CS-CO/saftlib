...
void EB_Forward::open_pts()
{
  ...
  grantpt( _pts_fd );
  unlockpt( _pts_fd );
  chmod( ptsname( _pts_fd ), S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH );

  // OLD_DEBUG: std::cerr << "eb-forward " << eb_forward_path() << std::endl;
  io_source = saftbus::Loop::get_default().connect<saftbus::IoSource>(
      std::bind( &EB_Forward::accept_connection, this, std::placeholders::_1 ), _pts_fd, POLLIN );
}
  ...
  for ( ;; )
  {
    ...
    { // normal read
      ...
      if ( request.size() == 4 )
      {
        ...
        //&& request[2]     == 0x11
        //&& (request[3]     == 0xff || request[3] == 0x77)) // on 32 bit systems, the host will send 0x77 while on 64
        //bit systems the host will send 0xff
        {
          // hard-coded response
        ...
        { // assume it is an Etherbone cycle header, calculate the response size
