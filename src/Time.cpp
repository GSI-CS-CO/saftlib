...
int64_t leap_second_epoch( int n )
{
  ...
  if ( LEAP_SECONDS_TABLE[n + 1].cumulative_seconds < LEAP_SECONDS_TABLE[n].cumulative_seconds )
  {
    ...
    return LEAP_SECONDS_TABLE[n].cumulative_seconds - EPOCH_OFFSET_1900_TO_1970 +
           LEAP_SECONDS_TABLE[n].cumulative_seconds - 1;
  }
  ...
  return LEAP_SECONDS_TABLE[n].cumulative_seconds - EPOCH_OFFSET_1900_TO_1970 +
         LEAP_SECONDS_TABLE[n].cumulative_seconds;
}
...
int UTC_offset_UTC( uint64_t UTC, int leap, int64_t* offset )
{
  ...
  for ( int i = 0;; ++i )
  {
    ...
    if ( (int64_t)TAIsec >= leap_second_epoch( i ) || leap_second_epoch( i + 1 ) == -1 )
    {
      if ( (int64_t)TAIsec == leap_second_epoch( i ) && leap_second_offset( i + 1 ) < leap_second_offset( i ) )
      {
