...
static void help( void )
{
  ...
  std::cout
      ...
      << std::endl;
  std::cout << "  -X                   use development mode to inject without requiring a WhiteRabbit lock"
            << std::endl;
  std::cout << "  -v                   print sleep time, inject time, scheduled time for each event" << std::endl;
  std::cout << std::endl;
  std::cout << "  -n N                 N (1..) is number of iterations of the schedule. If unspecified, N is set to 1"
            << std::endl;
  std::cout << std::endl;

  std::cout << "  The file must contain exactly one message per line. Each line must have the following format:"
            << std::endl;
  std::cout << "  '<eventID> <param> <time>', example: '0x1111000000000000 0x0 123000000', time [ns] and decimal."
            << std::endl;
  std::cout << std::endl;
  std::cout
      << "This tool provides a primitive Data Master for local operations in the FEC. Timing messages are injected"
      << std::endl;
  std::cout
      << "at the input of the ECA. This allows for scheduling actions in hard real-time down to ns. This tool might"
      << std::endl;
  std::cout << "be useful when a central Data Master is not available, for rapid prototyping or tests." << std::endl
            << std::endl;
...
int main( int argc, char** argv )
{
  ...
  if ( optind + 1 < argc )
    ...
    std::cerr << program << ": expecting non-optional argument <file name> " << std::endl;
  if ( strlen( filename ) == 0 )
    std::cerr << program << ": illegal file name" << std::endl;
  ...
  try
  {
    ...
    // print headline
    uint8_t width = 53;
    std::cout << std::setfill( ' ' ) << std::setw( 15 ) << "Event time" << std::setw( 19 ) << "Event ID"
              << std::setw( 19 ) << "Parameter";
    if ( verboseMode )
    {
      std::cout << std::setfill( ' ' ) << std::setw( 15 ) << "Sleep time" << std::setw( 20 ) << "Inject event"
    ...
    for ( i = 0; i < nIter; i++ )
    {
      ...
      if ( ppsAlign )
        startTime = ( wrTime - ( wrTime.getTAI() % 1000000000 ) ) + 1000000000; // align schedule to next PPS
      else
        startTime = wrTime; // align schedule to current WR time
      ...
      if ( myfile.is_open() )
      {
        while ( getline( myfile, line ) )
        {
          ...
          std::cout << std::dec << std::setfill( ' ' ) << std::setw( 15 ) << eventTime << std::hex << " 0x"
                    << std::setw( 16 ) << eventID << " 0x" << std::setfill( '0' ) << std::setw( 16 ) << eventParam;
          // scheduled time for next inject
          nextTimeWR = startTime + eventTime;
          // sleep before injecting: keep the queue inside ECA short.
          // lets sleep until 100ms before event is due - required to avoid overrun (when iterating) or late actions
          // (for long schedules)
          wrTime = receiver->CurrentTime( devInject );
          if ( nextTimeWR > ( wrTime + 100000000 ) )
          { // only sleep if injected event "was" more than 100ms in the future
            sleepTime = (int64_t)( ( nextTimeWR - wrTime ) / 1000 ) -
                        100000; // sleep 100ms less than interval to injected event
            usleep( sleepTime );
          } // if next time
          ...
          if ( verboseMode )
          {
            std::cout << std::dec << std::setfill( ' ' ) << std::setw( 15 ) << sleepTime * 1000 << " "
                      << std::setw( 19 ) << wrTime.getTAI() << " " << std::setw( 19 ) << nextTimeWR.getTAI();
          }
          std::cout << std::endl;
          sleepTime = 0;
        } // while getline
