...
static void io_help (void)
{
  ...
  std::cout << "  -s 0/1: Toggle special functionality" << std::endl;
...
int main (int argc, char** argv)
{
  ...
  int  io_term      = 1;     /* Termination resistor, should be enabled by default */
  int  io_spec      = 0;     /* Special function */
  ...
  /* Parse for options */
  while ((opt = getopt(argc, argv, "o:t:s:m:d:vhi")) != -1)
  {
    switch (opt)
    {
      case 'o': { io_oe        = atoi(optarg); set_oe    = true; break; }
      case 't': { io_term      = atoi(optarg); set_term  = true; break; }
      case 's': { io_spec      = atoi(optarg); set_spec  = true; break; }
  ...
  else
  {
    ...
    if (io_term > 1  || io_term < 0)  { std::cout << "Error: Termination resistor setting is invalid" << std::endl;  show_help = true; return_code = __IO_RETURN_FAILURE; }
    if (io_spec > 1  || io_spec < 0)  { std::cout << "Error: Special function setting is invalid!" << std::endl;     show_help = true; return_code = __IO_RETURN_FAILURE; }
    ...
    { 
      ...
      else           { return_code = io_setup(io_oe, io_term, io_spec, io_mux, io_drive, set_oe, set_term, set_spec, set_mux, set_drive, verbose_mode); }
    }
  }
