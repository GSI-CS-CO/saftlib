...
#define WR_MIL_GW_CMD_TEST                                                                                             \
  0x5 // command that does nothing. it is useful to initiate to see if the firmware cleans the command register (see if
      // the firmware runs at all)
...
#define WR_MIL_GW_REG_TRIG_UTC_DELAY                                                                                   \
  0x10 // delay [us] between the trigger event and the first UTC (and other) generated events
#define WR_MIL_GW_REG_EVENT_SOURCE                                                                                     \
  0x14 // for internal use: register to hold the source configuration: 1 = SIS ; 2 = ESR ; 0 not configured
#define WR_MIL_GW_REG_LATENCY                                                                                          \
  0x18 // MIL event is generated 100us+latency after the WR event. The value of latency can be negative
...
#define WR_MIL_GW_REG_REQUEST_FILL_EVT                                                                                 \
  0x47c // if this is written to 1, the gateway will send a fill event as soon as possible
