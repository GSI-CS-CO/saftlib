...
#define ECA_QUEUE_QUEUE_ID_GET                                                                                         \
  0x00 // ro,  8 b, The index of a_channel_o from the ECA to which this queue is connected (set
       // channel_select=queue_id+1)
#define ECA_QUEUE_POP_OWR 0x04 // wo,  1 b, Pop action from the channel's queue
#define ECA_QUEUE_FLAGS_GET                                                                                            \
  0x08 // ro,  5 b, Error flags for this action(0=late, 1=early, 2=conflict, 3=delayed, 4=valid)
