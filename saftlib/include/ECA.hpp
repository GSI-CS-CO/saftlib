...
/// ECA is a hardwar unit cabable of executing actions at a given time (1 ns resolution) in response to events that meet
/// a condition. An event contains a 64-bit timestamp, 64-bit id, 64-bit flags A condition contains a 64-bit id, 64-bit
/// prefix-mask, 64-bit time offset Action can be one of the following:
///   - Output: change output driver (rising or falling edge on one of the GPIOs)
///   - SoftwareActionSink: send the content of the event to host system (store the event content in some registers and
///   send an MSI so that the host can read the event information)
...
///
/// Hardware supports are up to 32 SoftwareActionSink at the same time. The hardware has one single Output channel
/// configured for the host system with 32 sub-channels for the different SoftwareActionSinks. Each SoftwareActionSinks
/// occupies one of the sub-channels. This implementation uses an std::vector< std::unique_ptr< ActionSink> > which is
/// resized in the constructor to the number of available sub-channels. Each invalid unique_ptr<ActionSink> indicates an
/// un-occupied sub-channel. Each SoftwareActionSink can have many conditions. On execution, ECA writes a tag value into
/// the ActionSink that refers to the condition that matched the incoming event.
class ECA : public MsiDevice
{
  ...
  std::map<std::string, std::string> scubus_action_sinks; // a list of scubus_action_sinks that is created on
                                                          // construction and returned by getSCUbusActionSinks()
  std::map<std::string, std::string> ecpu_action_sinks;   // a list of ecpu_action_sinks that is created on construction
                                                          // and returned by getEmbeddedCPUActionSinks()
