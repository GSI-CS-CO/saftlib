...
static int         bg_config_io( uint32_t t_high,
                                 uint32_t t_period,
                                 int64_t  t_burst,
                                 uint64_t b_delay,
                                 uint32_t b_flag,
...
static int         bg_create_burst( int      burst_id,
                                    uint64_t e_id,
                                    uint64_t e_mask,
                                    uint64_t stop_e_id,
                                    uint64_t stop_e_mask,
                                    bool     verbose_mode );
...
static int
bg_invoke_async( std::shared_ptr<BurstGenerator_Proxy> bg, uint32_t inst_code, std::vector<uint32_t> inst_args );
...
static int bg_do_configuration( void )
{
  ...
  if ( bg_fw_id )
  {
    return_code = bg_get_fw_id();
  }
  else if ( bg_state )
  {
    return_code = bg_print_state();
  }
  else if ( bg_instr )
  {
    return_code = bg_instruct( bg_instr_args );
  }
  else if ( bg_cfg_io )
  {
    return_code = bg_config_io( bg_t_high, bg_t_p, bg_t_b, bg_b_delay, bg_b_flag, verbose_mode );
  }
  else if ( bg_clr_all )
  {
    return_code = bg_clear_all( verbose_mode );
  }
  else if ( bg_ls_bursts )
  {
    return_code = bg_list_bursts( bg_id, verbose_mode );
  }
  else if ( bg_rm_burst )
  {
    return_code = bg_remove_burst( bg_id, verbose_mode );
  }
  else if ( bg_disen_burst )
  {
    return_code = bg_disenable_burst( bg_id, bg_disen, verbose_mode );
  }
  else if ( bg_mk_burst )
  {
    return_code =
        bg_create_burst( burstId, bg_start_e_id, bg_start_e_mask, bg_stop_e_id, bg_stop_e_mask, verbose_mode );
  }
  else if ( show_table )
  {
...
static int bg_check_arguments( void )
{
  ...
  if ( bg_mk_burst )
  {
    if ( bg_start_e_id == 0 )
    {
      std::cout << "Error: Invalid ID for the start event!" << std::endl;
      return ( __IO_RETURN_FAILURE );
    }
    if ( bg_start_e_id == bg_stop_e_id )
    {
      std::cout << "Error: IDs of the start and stop events must be different!" << std::endl;
      return ( __IO_RETURN_FAILURE );
    }
  }
...
static int parse_options( int optc, char* optv[], const char* optstr )
{
  ...
  while ( ( opt = getopt( optc, optv, optstr ) ) != -1 )
  {
    ...
    case 'p':
    {
      if ( optv[optind - 1] != NULL )
      {
        bg_t_high = strtoul( optv[optind - 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing active state length of a pulse (u32)!" << std::endl;
        return -1;
      }
      if ( optv[optind + 0] != NULL )
      {
        bg_t_p = strtoul( optv[optind + 0], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing signal period (u32)!" << std::endl;
        return -1;
      }
      if ( optv[optind + 1] != NULL )
      {
        bg_t_b = strtoll( optv[optind + 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing burst period (i64)!" << std::endl;
        return -1;
      }
      if ( bg_t_b > 0 && (uint64_t)bg_t_b < BG_BURST_PERIOD )
      {
        std::cerr << "Error: Burst period exceeds the lower limit of " << BG_BURST_PERIOD << " ns!" << std::endl;
        return -1;
      }
      if ( bg_t_high > bg_t_p )
      {
        std::cerr << "Error: Invalid signal parameters, t_high > t_p" << std::endl;
        return -1;
      }
      if ( bg_t_b > 0 && bg_t_p > bg_t_b )
      {
        std::cerr << "Error: Invalid burst parameters, t_p > t_b" << std::endl;
        return -1;
      }
      if ( optv[optind + 2] != NULL )
      {
        bg_b_delay = strtoull( optv[optind + 2], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing burst delay!" << std::endl;
        return -1;
      }
      if ( optv[optind + 3] != NULL )
      {
        bg_b_flag = strtoul( optv[optind + 3], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing burst flag!" << std::endl;
        return -1;
      }
      if ( bg_b_flag > 1 )
      {
        std::cerr << "Error: Invalid burst flag! Valid values: 0|1" << std::endl;
        return -1;
      }
      bg_cfg_io = true;
      break;
    ...
    case 'e':
    {
      if ( optv[optind - 1] != NULL )
      {
        bg_id = strtol( optv[optind - 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing burst id!" << std::endl;
        return -1;
      }
      if ( bg_id < 0 || bg_id > N_BURSTS )
      {
        std::cerr << "Error: invalid burst id, must be 0 <= id <= " << N_BURSTS << std::endl;
        return -1;
      }
      if ( optv[optind + 0] != NULL )
      {
        bg_disen = strtol( optv[optind + 0], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing dis/enable setting!" << std::endl;
        return -1;
      }
      bg_disen_burst = true;
      break;
    ...
    case 'i':
    {
      ...
      while ( index < optc )
      {
        if ( optv[index] != NULL && optv[index][0] != '-' )
        {
          bg_instr_args.push_back( optv[index] );
          ++index;
        }
        else
        {
    ...
    case 'b':
    {
      if ( optv[optind - 1] != NULL )
      {
        burstId = strtol( optv[optind - 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing burst id!" << std::endl;
    ...
    case 's':
    {
      if ( optv[optind - 1] != NULL )
      {
        bg_start_e_id = strtoull( optv[optind - 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing an ID for the start event!" << std::endl;
        return -1;
      }
      bg_mk_burst = true;
      break;
    ...
    case 't':
    {
      if ( optv[optind - 1] != NULL )
      {
        bg_stop_e_id = strtoull( optv[optind - 1], &pEnd, 0 );
      }
      else
      {
        std::cerr << "Error: Missing an ID for the stop event!" << std::endl;
        return -1;
      }
      break;
    }
    ...
    case 'o':
    {
      if ( optv[optind - 1] != NULL )
      {
        optFileName = string( optv[optind - 1] );
      }
      else
      {
...
bg_invoke_async( std::shared_ptr<BurstGenerator_Proxy> bg, uint32_t inst_code, std::vector<uint32_t> inst_args )
{
  ...
  {
    if ( saftlib::wait_for_signal( 1000 ) ==
         0 )                      // wait for response or time-out (burst generator checks its mailbox every second)
  ...
  {
    std::cerr << "Failed to execute an instruction: " << std::hex << ret_code << " (expected: " << inst_code << ")"
              << std::endl;
    return -1;
  }
...
static void print_burst_info( std::vector<uint32_t> info )
{
  ...
  std::cout << " id = " << info.at( 0 ) << ", io type = " << info.at( 1 ) << ", io index = " << info.at( 2 )
            << ", start = " << std::hex << info.at( 3 ) << ':' << info.at( 4 ) << ", stop = " << info.at( 5 ) << ':'
            << info.at( 6 ) << ", cycle = " << info.at( 7 ) << ':' << info.at( 8 )
                         ...
                         bool     verbose_mode )
{
  ...
  else if ( t_period < BG_BURST_PERIOD ) // pulse period is shorter than block production period
  {
    ...
    uint32_t n_rest =
        ( t_rem - n_pulses * t_period ) / t_high; // the rest of burst that can be expressed by a condition
  ...
  if ( t_burst > 0 )
  { // finite burst -> determine a number of blocks
  ...
  if ( verbose_mode )
  {
    ...
    std::cout << "+++ config:" << std::endl
              ...
              << " n_block=" << std::dec << n_block << " (" << std::hex << n_block << ") # of blocks for loop "
              << std::endl
              << " n_main_conditions=" << std::dec << n_main_conditions << " (" << std::hex << n_main_conditions
              << ") main pulses" << std::endl
              << " n_rem_conditions=" << std::dec << n_rem_conditions << " (" << std::hex << n_rem_conditions
              << ") remaining pulses" << std::endl
              << " n_conditions=" << std::dec << n_conditions << " (" << std::hex << n_conditions
              << ") total ECA conditions" << std::endl;
  }
  ...
  try
  {
    ...
    {
      std::cerr << "Not enough space is available for the desired conditions! Needs: " << n_conditions
                << ", available: " << eca_entries << std::endl;
      return -1;
    }
  ...
  if ( n_conditions == 1 )
  {
    ...
    if ( result != 0 )
    {
      std::cerr << "Failed to create a condition for burst: id = " << std::hex << _e_id << " mask = " << _e_mask
                << std::endl;
      io_destroy( true ); // destroy conditions that were created
      return result;
    }
    else if ( verbose_mode )
      std::cout << "id = " << std::hex << _e_id << "msk = " << _e_mask << std::dec << " offset = " << offset
  ...
  else
  {
    ...
    for ( uint32_t i = 0; i < n_conditions; i++ )
    {
      ...
      if ( result != 0 )
      {
        std::cerr << "Failed to create conditions for burst head: id = " << std::hex << _e_id << " mask = " << _e_mask
                  << std::endl;
        io_destroy( true ); // destroy conditions that were created
        return result;
      }
    ...
    if ( n_block )
    {
      ...
      for ( uint32_t i = 0; i < n_main_conditions; i++ )
      {
        ...
        if ( result != 0 )
        {
          std::cerr << "Failed to create conditions for burst body: id = " << std::hex << _e_id << " mask = " << _e_mask
                    << std::endl;
          io_destroy( true ); // destroy conditions that were created
          return result;
        }
  ...
  try
  {
    ...
    if ( ecpu )
    {
      ...
      for ( unsigned int c = 0; c < n_conditions.size(); c++ )
      {
        ...
        if ( c_proxy )
        {
          if ( c_proxy->getID() == e_id && c_proxy->getMask() == e_mask && c_proxy->getTag() == tag &&
               c_proxy->getOffset() == offset )
          {
...
static void io_help( void )
{
  ...
  std::cout
      << "  -i <instr>:                         Send an user instruction to the burst generator, allowed instructions are listed below:"
      << std::endl;
  std::cout << "      instr                             User instruction. Valid codes:" << std::endl;
  std::cout
      << "                                        1 = configure the burst generator (switch to the 'configure' state)"
      << endl;
  std::cout << "                                        2 = start the operation (switch to the 'opready' state)"
            << endl;
  std::cout << "                                        3 = stop the operation (switch back to the 'configure' state)"
            << endl;
  std::cout << std::endl;
  std::cout << "Options for the 'configure' state:" << std::endl;
  ...
  std::cout << "  -s <start>:                         Specify ID of the start event. Requires -n and -b options!"
            << std::endl;
  std::cout << "  -t <stop>:                          Specify ID of the stop event. Can be used with the -s option"
            << std::endl;
  std::cout << "      start, stop                       IDs of the start and stop events" << std::endl;
  std::cout
      << "  -p <t_hi> <t_p> <b_p> <b_d> <b_f>:  Define signal parameters to a new/existing burst. Requires -b option!"
      << std::endl;
  std::cout << "      t_hi, t_p                         Signal high width (ns), signal period (ns)" << std::endl;
  std::cout
      << "      b_p, b_d, b_f                     Burst period (ns, 0=infinite), burst delay (ns), burst flag (0=new/overwrite, 1=append)"
      << std::endl;
  std::cout << "                                        IMPORTANT: minimum burst period is " << BG_BURST_PERIOD << " ns"
            << std::endl;
  std::cout << "  -l <0 | id>:                        List burst(s): 0 for burst IDs, otherwise burst info"
            << std::endl;
  std::cout
      << "  -e <id> <ctrl>:                     Enable burst: enable if 'ctrl' is non-zero, otherwise disable (ctrl=0)"
      << std::endl;
  std::cout << "  -r <id>:                            Remove burst" << std::endl;
  std::cout << std::endl;
  // TODO: Why was the following block deactivated? Is its function still needed?
  /*
  std::cout << "      0x1                                        Print the burst parameters. Arguments: burst_id" <<
  std::endl; std::cout << "      0x2                                        Obtain the burst parameters. Arguments:
  burst_id, delay, conditions, block period, flag, verbose" << std::endl; std::cout << "      0x3 Obtain the block
  production parameters. Arguments: burst_id, n_block_h32, n_block_l32, verbose" << std::endl; std::cout << "      0x10
  Print MSI configuration" << std::endl; std::cout << "      0x11                                       Print ECA
  channel counters" << std::endl; std::cout << "      0x12                                       Print ECA queue
  content" << std::endl; std::cout << "  All print instructions require the eb-console tool to be run to see the output"
  << std::endl; std::cout << "  -x:                                            Clear all unowned conditions for the IO
  and eCPU actions." << std::endl; std::cout << "  -o <file>:                                     Load options from a
  given <file>" << std::endl;*/
  std::cout << std::endl;
  std::cout << "Examples (valid only in the 'configure' state):" << std::endl << std::endl;
  ...
  std::cout
      << "   Show the info of a burst with ID 1. It includes the IO port name, IDs of the start and stop events, enable state."
      << std::endl
      << std::endl;
  std::cout << program << " tr0" << " -n B1" << " -b 1" << " -s 0x1234567800000000" << std::endl;
  std::cout
      << "   Create new burst at the output port B1. The burst's ID is 1, the burst is started by a timing message with the event ID of 0x1234567800000000."
      << std::endl
      << std::endl;
  std::cout << program << " tr0" << " -b 1" << " -p 1000000 2000000 4000000 0 0" << std::endl;
  std::cout
      << "   Define signal for the burst with ID 1. The given burst must have already been created! Signal high width is 1 ms, signal period is 2 ms, burst length is 4 ms, no delay."
      << std::endl
      << std::endl;
      ...
      if ( ( ( ioNameGiven && ( it->first == ioName ) ) || !ioNameGiven ) )
      {
        ...
        if ( !mode )
        {
          if ( !setup_only )
          {
            ...
            proxies.push_back( SoftwareCondition_Proxy::create(
                sinks.back()->NewCondition( true, prefix, -2, IO_CONDITION_OFFSET ) ) );
                     ...
                     bool verbose_mode )
{
  ...
  try
  {
    ...
    if ( io_set )
    {
      ...
      if ( set_stime )
      {
        input_proxy->setStableTime( s_time );
      }
    }
    else
    {
