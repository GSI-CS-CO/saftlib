    ...
    , device( tr->OpenDevice::get_device() )
{
  ...
  std::cerr << "BurstGenerator: subscribed mailbox" << std::hex << " slot: " << my_slot->getIndex()
            << " addr: " << my_slot->getAddress() << " path: " << my_msi->address() << std::endl;
  ...
  for ( unsigned i = 0; i < tr->LM32Cluster::getCpuCount(); ++i )
  {
    ...
    if ( shm_buffer.at( SHM_BUF_IDX::COMMON_END ) > ( ram_base + SHM_BASE ) )
    {
      std::cerr << "BurstGenerator: failure in shared memory layout (common-lib overlaps app-spec section): "
                << std::hex << shm_buffer.at( SHM_BUF_IDX::COMMON_END ) << " > " << std::hex << ( ram_base + SHM_BASE )
                << std::endl;
      shm_buffer.clear();
      continue;
    }
    ...
    std::cerr << "BurstGenerator: LM32 ram base = 0x" << std::hex << ram_base
              << ", my mailbox slot = " << my_slot->getIndex() << " is stored at 0x" << std::hex
              << (uint32_t)( ram_base + SHM_MB_SLOT_HOST ) << " in the shared memory for LM32." << std::endl;

    std::cerr << "BurstGenerator: LM32 COMMON_BEGIN = 0x" << std::hex << shm_buffer.at( SHM_BUF_IDX::COMMON_BEGIN )
              << ", COMMON_END = 0x" << std::hex << shm_buffer.at( SHM_BUF_IDX::COMMON_END ) << ", COMMON_CMD = 0x"
              << std::hex << shm_buffer.at( SHM_BUF_IDX::COMMON_CMD ) << ", COMMON_STATE = 0x" << std::hex
              << shm_buffer.at( SHM_BUF_IDX::COMMON_STATE ) << ", CMD_ARGS at 0x" << std::hex
              << shm_buffer.at( SHM_BUF_IDX::CMD_ARGS ) << std::endl;
  ...
  std::cerr << "BurstGenerator: bg mailbox addr = " << bg_slot << ", lm32 cpu index = " << cpu_idx << ", ram base = 0x"
            << std::hex << ram_base << std::endl;
}
...
int32_t BurstGenerator::instruct( uint32_t code, const std::vector<uint32_t>& args )
{
  ...
  try
  {
    ...
    std::cerr << "BurstGenerator: method call instruct(0x" << std::hex << code << std::dec << ") succeeded."
              << std::endl;
    return EB_OK;
  }
