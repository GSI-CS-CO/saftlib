...
SoftwareActionSink::SoftwareActionSink( ECA&                eca,
                                        ...
                                        unsigned            channel,
                                        unsigned            num,
                                        eb_address_t        queue_address,
                                        saftbus::Container* container )
    : ActionSink( eca, obj_path, name, channel, num, container )
    , queue( queue_address )
{
}
...
void SoftwareActionSink::receiveMSI( uint8_t code )
{
  ...
  if ( code == ECA_VALID )
  {
    ...
    eb_data_t flags, rawNum, event_hi, event_lo, param_hi, param_lo, tag, tef, deadline_hi, deadline_lo, executed_hi,
        executed_lo;
    ...
    {
      std::cerr
          << "SoftwareActionSink: MSI for increase in VALID_COUNT did not correspond to a valid action in the queue"
          << std::endl;
      return;
    }
