...
void SerdesClockGen::CalcClockParameters( double hi, double lo, uint64_t phase, struct SerClkGenControl* control )
{
  ...
  control->bit_pattern_skip =
      ( control->bit_pattern_normal & high_mask ) | ( ( control->bit_pattern_normal & low_mask ) >> 1 );
#if IO_SER_CLK_GEN_DEBUG_MODE
...
bool SerdesClockGen::StartClock( int      io_channel,
                                 int      io_index,
                                 double   high_phase,
                                 double   low_phase,
...
bool SerdesClockGen::ConfigureClock( int      io_channel,
                                     int      io_index,
                                     double   high_phase,
                                     double   low_phase,
                                     uint64_t phase_offset )
