...
extern "C" void create_services( saftbus::Container* container, const std::vector<std::string>& args )
{
  ...
  for ( auto& device : args )
  {
    ...
    std::unique_ptr<saftlib::FunctionGeneratorFirmware> fw(
        new saftlib::FunctionGeneratorFirmware( container, saftd, tr ) );
    ...
    {
      auto service =
          std::unique_ptr<saftlib::FunctionGeneratorFirmware_Service>( new saftlib::FunctionGeneratorFirmware_Service(
