...
FunctionGeneratorFirmware::FunctionGeneratorFirmware( saftbus::Container* cont,
                                                      SAFTd*              saft_daemon,
    ...
    , device( tr->OpenDevice::get_device() )
{
  ...
  if ( !have_fg_firmware )
    throw saftbus::Error( saftbus::Error::FAILED, "no FunctionGeneratorFirmware found" );
}
...
void FunctionGeneratorFirmware::clear()
{
  ...
  try
  {
    ...
    for ( auto& fg : fgs )
      if ( fg.second )
        fg_names.push_back( fg.second->getObjectPath() );
    std::string mfg_name;
    if ( mfg )
      mfg_name = mfg->getObjectPath();
    fgs.clear();
    mfg.reset();
    addon_objects.clear();
    if ( container )
    {
      ...
      if ( mfg_name.size() )
        container->remove_object( mfg_name );
    }
  }
  catch ( ... )
  { // catch exceptions that occur when the object was removed before
    // nothing.
  }
}
...
void FunctionGeneratorFirmware::firmware_rescan( int host_to_lm32_mailbox_slot_idx )
{
  ...
  if ( version == 3 )
    device.write( fgb + SHM_BASE + FG_SCAN_DONE, EB_DATA32, 1 );
  if ( version == 4 )
    device.write( fgb + SHM_BASE + FG_BUSY, EB_DATA32, 1 );
  ...
  for ( int i = 0;; ++i )
  {
    eb_data_t done;
    if ( version == 3 )
      device.read( fgb + SHM_BASE + FG_SCAN_DONE, EB_DATA32, &done );
    if ( version == 4 )
      device.read( fgb + SHM_BASE + FG_BUSY, EB_DATA32, &done );
    if ( done == 0 )
    {
      break;
    }
    if ( i == 200 )
    { // wait at least 20 seconds before timeout
...
std::map<std::string, std::string> FunctionGeneratorFirmware::ScanMasterFg()
{
  ...
  if ( !nothing_runs() )
  {
    throw saftbus::Error( saftbus::Error::ACCESS_DENIED,
                          "FunctionGeneratorFirmware::Scan is not allowed if any channel is active" );
  }
  ...
  if ( have_fg_firmware )
  {
    ...
    std::shared_ptr<std::vector<int>> allocation( new std::vector<int> );
    ...
    for ( unsigned j = 0; j < FG_MACROS_SIZE; ++j )
    {
      ...
      functionGeneratorImplementations.push_back( std::make_shared<FunctionGeneratorImpl>(
          saftd, tr, allocation, fgb, mb_slot, num_channels, buffer_size, j, macros[j] ) );

      // std::ostringstream spath;
      // spath.imbue(std::locale("C"));
      // spath << objectPath << "/fg_" << j;
      // std::string path = spath.str();
      // FunctionGeneratorImpl::ConstructorType args = { path, tr, allocation, fgb, swi, sdb_msi_base, mailbox,
      // (unsigned)num_channels, (unsigned)buffer_size, j, (uint32_t)macros[j] };
    ...
    if ( container )
    {
      std::unique_ptr<MasterFunctionGenerator_Service> service(
          new MasterFunctionGenerator_Service( mfg.get(), std::bind( &FunctionGeneratorFirmware::clear, this ) ) );
  ...
  if ( have_fg_firmware )
  {
    ...
    for ( unsigned j = 0; j < FG_MACROS_SIZE; ++j )
    {
      ...
      std::shared_ptr<FunctionGeneratorImpl> fg_impl = std::make_shared<FunctionGeneratorImpl>(
          saftd, tr, allocation, fgb, mb_slot, num_channels, buffer_size, j, macros[j] );
      // FunctionGeneratorImpl::ConstructorType args = { path, tr, allocation, fgb, swi, sdb_msi_base, mailbox,
      // (unsigned)num_channels, (unsigned)buffer_size, j, (uint32_t)macros[j] }; FunctionGenerator::ConstructorType
      // fgargs = { path, tr, std::make_shared<FunctionGeneratorImpl>(args) }; std::shared_ptr<FunctionGenerator> fg =
      // FunctionGenerator::create(fgargs);

      std::ostringstream name;
      name << "fg-" << (int)fg_impl->getSCUbusSlot() << "-" << (int)fg_impl->getDeviceNumber();

      std::shared_ptr<FunctionGenerator> fg =
          std::make_shared<FunctionGenerator>( container, name.str(), path, fg_impl );
