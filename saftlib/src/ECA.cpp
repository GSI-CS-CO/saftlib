  ...
  for ( unsigned channel_idx = 1; channel_idx < channels; ++channel_idx )
  {
    ...
    case ECA_LINUX:
      ...
      if ( ECA_LINUX_channel == nullptr )
      {
        ...
        ECA_LINUX_channel->resize(
            raw_max_num ); // create enough slots for as much SoftwareActionSinks as the hardware supports
      }
      else
      {
        std::cerr << "more than one Linux facing ECA channel. We will use the first of them and ignore the rest"
                  << std::endl;
      }
      break;
    case ECA_WBM:
    {
      ...
      if ( acwbm.size() == 1 )
      {
        ...
        std::unique_ptr<WbmActionSink> wbm_sink( new WbmActionSink(
            device, *this, path, "acwbm", channel_idx, (eb_address_t)acwbm[0].sdb_component.addr_first, container ) );
        if ( container )
        {
    ...
    case ECA_SCUBUS:
    {
      ...
      if ( scubus.size() == 1 )
      {
        ...
        std::unique_ptr<SCUbusActionSink> scubus_sink( new SCUbusActionSink(
            device, *this, path, "scubus", channel_idx, (eb_address_t)scubus[0].sdb_component.addr_first, container ) );
        if ( container )
        {
    ...
    case ECA_EMBEDDED_CPU:
    {
      ...
      for ( unsigned queue_id = 1; queue_id < channels; ++queue_id )
      {
        ...
        cycle.close();
        // OLD_DEBUG: std::cerr << "ECA: Found queue @ 0x" << std::hex << queue_addresses[queue_id] << std::dec <<
        // std::endl; OLD_DEBUG: std::cerr << "ECA: Found queue with ID: " << get_id << std::endl;
        if ( get_id == ECA_EMBEDDED_CPU )
        {
          ...
          std::unique_ptr<EmbeddedCPUActionSink> ecpu_sink(
              new EmbeddedCPUActionSink( *this, path, "embedded_cpu", channel_idx, container ) );
          if ( container )
          {
            std::unique_ptr<EmbeddedCPUActionSink_Service> service(
                new EmbeddedCPUActionSink_Service( ecpu_sink.get() ) );
...
static bool operator<( const ECA_OpenClose& a, const ECA_OpenClose& b )
{
  ...
  if ( a.key > b.key )
    return false;
  if ( !a.open && b.open )
    return true; // close first
  if ( a.open && !b.open )
    return false;
  if ( a.subkey > b.subkey )
    return true; // open largest first, close smallest first
  if ( a.subkey < b.subkey )
    return false;
  ...
  for ( auto& channel : ECAchannels )
  {
    for ( auto& actionSink : channel )
    {
      ...
      for ( auto& number_condition : actionSink->getConditions() )
      {
        ...
        if ( caller.empty() || condition->getOwner() == caller )
        {
          if ( condition->getActive() )
            deactivated.push_back( condition.get() );
...
void ECA::compile()
{
  ...
  // 		clog << kLogDebug << "W: " << walk[i].next << " " << walk[i].offset << " " << walk[i].tag << " " <<
  // walk[i].flags << " " << (int)walk[i].channel << " " << (int)walk[i].num << std::endl; #endif
  ...
  for ( unsigned i = 0; i < walk.size(); ++i )
  {
    ...
    cycle.write(
        adr_first + ECA_WALKER_RW_OFFSET_HI_RW, EB_DATA32, (uint64_t)we.offset >> 32 ); // don't sign-extend on shift
    ...
    , sas_count( 0 )
{
  ...
  {
    channel_irqs.push_back(
        saftd.request_irq( *this, std::bind( &ECA::msiHandler, this, std::placeholders::_1, channel_idx ) ) );
...
{
  for ( auto& output : ECAchannels[0] )
  { // outputs are always on channel 0
...
std::string ECA::NewSoftwareActionSink( const std::string& name_ )
{
  ...
  // if (container) {
  // 	std::cerr << "TimingReceiver::NewSoftwareActionSink client_id = " << container->get_calling_client_id() <<
  // std::endl;
  // }
  ...
  // OLD_DEBUG: std::cerr << "ECA::NewSoftwareActionSink:   find 1 in " << std::dec << ECA_LINUX_channel->size() <<
  // "slots" << std::endl;
  unsigned num = 0;
  for ( auto& softwareActionSink : *ECA_LINUX_channel )
  {
  ...
  if ( num >= ECA_LINUX_channel_subchannels )
  {
    throw saftbus::Error( saftbus::Error::INVALID_ARGS,
                          "All available Linux facing ECA subchannels are already in use" );
  }
  ...
  // OLD_DEBUG: std::cerr << "NewSoftwareActionSink: channel = " << channel << " num = " << num << "
  // queue_addresses.size() = " << queue_addresses.size() << std::endl;
  ...
  std::unique_ptr<SoftwareActionSink> software_action_sink(
      new SoftwareActionSink( *this, path, name, channel, num, address, container ) );
  std::string sink_object_path = software_action_sink->getObjectPath();
  if ( container )
  {
    std::unique_ptr<SoftwareActionSink_Service> service( new SoftwareActionSink_Service(
        software_action_sink.get(), std::bind( &ECA::removeSowftwareActionSink, this, software_action_sink.get() ) ) );
