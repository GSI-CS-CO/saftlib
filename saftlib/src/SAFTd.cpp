...
eb_status_t SAFTd::write( eb_address_t address, eb_width_t width, eb_data_t data )
{
  ...
  if ( it != irqs.end() )
  {
    ...
    {
      std::cerr << "Unhandled unknown exception in MSI handler for 0x" << std::hex << address << std::dec << std::endl;
    }
  }
  else
  {
...
std::string SAFTd::AttachDevice( const std::string& name, const std::string& etherbone_path, int polling_interval_ms )
{
  ...
  try
  {
    ...
    if ( container )
    {
      std::unique_ptr<TimingReceiver_Service> service(
          new TimingReceiver_Service( timing_receiver, std::bind( &SAFTd::RemoveObject, this, name ), false ) );
...
//
// if a hardware MSI master writes to 0x10400, the first  etherbone master (connected to dev/wbm0) will get a callback
// on address 0x400 if a hardware MSI master writes to 0x11400, the second etherbone master (connected to dev/wbm0) will
// get a callback on address 0x400 if a hardware MSI master writes to 0x20400, another etherbone master connected via
// usb will get the MSI on address 0x400 if ((msi_first & size_mask) != 0) { 	throw
// etherbone::exception_t("request_irq/misaligned", EB_FAIL);
// }
  ...
  for ( int i = 0; i < 2000; ++i )
  {
    ...
    if ( request_irq( irq_adr, slot ) )
    {
      ...
      return std::unique_ptr<IRQ>(
          new IRQ( this, irq_adr, msi.msi_device.msi_first ) ); // return the adress that triggers the msi
    }
  }
...
TimingReceiver* SAFTd::getTimingReceiver( const std::string& tr_obj_path )
{
  ...
  if ( ( pos = tr_obj_path.find( object_path ) ) == tr_obj_path.npos )
  {
    const std::string& tr_name =
        tr_obj_path; // maybe the name of the TimingReceiver was given instead of its object_path
