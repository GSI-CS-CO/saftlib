...
FunctionGeneratorImpl::FunctionGeneratorImpl( SAFTd*                            saft_daemon,
    ...
    , resetTimeout()
    , startTag( 0 )
    , executedParameterCount( 0 )
    , fillLevel( 0 )
    , filled( 0 )
    , fifo( 16384 )
    , // a fifo entry is 12 bytes long.
    ...
    fg_fifo_max_size( 0 )
{
  ...
  if ( fg_fifo_max_size_env != nullptr )
  {
    ...
    if ( !in )
    {
      ...
      std::cerr << " cannot read initial capacity from environment variable: \'" << fg_fifo_max_size_env << "\'"
                << std::endl;
    }
    else
    {
...
FunctionGeneratorImpl::~FunctionGeneratorImpl()
{
  ...
  // LOGGING: add logging here ("",-1, channel);
...
void FunctionGeneratorImpl::fifo_push_back( const ParameterTuple& tuple )
{
  ...
  {
    std::cerr << "FunctionGeneratorImpl: change fifo capacity from " << std::dec << fifo.size() << " to "
              << 2 * fifo.size() << std::endl;
...
{
  // LOGGING: add logging here( DRIVER_LOG("channel",-1, channel);
...
void FunctionGeneratorImpl::refill( bool first )
{
  ...
  for ( unsigned i = 0; i < refill; ++i )
  {
    ...
    control = ( ( tuple.step & 0x7 ) << 0 ) | ( ( tuple.freq & 0x7 ) << 3 ) | ( ( tuple.shift_b & 0x3f ) << 6 ) |
...
void FunctionGeneratorImpl::irq_handler( eb_data_t msi )
{
  ...
  { // stopped?
    ...
    else
    {
      ...
      if ( !abort && !hardwareMacroUnderflow && !microControllerUnderflow )
      { // success => empty FIFO
        fillLevel = 0;
        fifo.clear();
      }
...
uint64_t ParameterTuple::duration() const
{
  ...
  static const uint64_t sample_len[8] = {
    // fixed in HDL
    62500, // 16kHz in ns
    31250, // 32kHz
    15625, // 64kHz
...
{
  // LOGGING: add logging here DRIVER_LOG("param.size()",-1, parameters.size());
...
bool FunctionGeneratorImpl::appendParameterSet( const std::vector<int16_t>&       coeff_a,
...
{
  // LOGGING: add logging here  DRIVER_LOG("coeff.size()",-1, coeff_a.size());

  // confirm lengths match
  unsigned len = coeff_a.size();

  if ( coeff_b.size() != len )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "coeff_b length mismatch" );
  if ( coeff_c.size() != len )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "coeff_c length mismatch" );
  if ( step.size() != len )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "step length mismatch" );
  if ( freq.size() != len )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "freq length mismatch" );
  if ( shift_a.size() != len )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "shift_a length mismatch" );
  if ( shift_b.size() != len )
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "shift_b length mismatch" );

  // validate data
  for ( unsigned i = 0; i < len; ++i )
  {
    if ( step[i] >= 8 )
      throw saftbus::Error( saftbus::Error::INVALID_ARGS, "step must be < 8" );
    if ( freq[i] >= 8 )
      throw saftbus::Error( saftbus::Error::INVALID_ARGS, "freq must be < 8" );
    if ( shift_a[i] > 48 )
      throw saftbus::Error( saftbus::Error::INVALID_ARGS, "shift_a must be <= 48" );
    if ( shift_b[i] > 48 )
      throw saftbus::Error( saftbus::Error::INVALID_ARGS, "shift_b must be <= 48" );
  }
...
void FunctionGeneratorImpl::flush()
{
  ...
  if ( enabled )
  {
    // LOGGING: add logging here  DRIVER_LOG("enabled_flush_impossible",-1,-1);
...
{
  // LOGGING: add logging here  DRIVER_LOG("channel",-1,channel);
  // LOGGING: add logging here  DRIVER_LOG("enabled",-1,enabled);
  return enabled;
}
...
{
  // LOGGING: add logging here  DRIVER_LOG("channel",-1,channel);
  // LOGGING: add logging here  DRIVER_LOG("armed",-1,armed);
  return armed;
}
...
{
  // LOGGING: add logging here DRIVER_LOG("channel",-1,channel);
  // LOGGING: add logging here DRIVER_LOG("running",-1,running);
  return running;
}
...
{
  // LOGGING: add logging here DRIVER_LOG("channel",-1,channel);
  // LOGGING: add logging here DRIVER_LOG("startTag",-1,startTag);
  return startTag;
}
...
{
  // LOGGING: add logging here DRIVER_LOG("channel",-1,channel);
  // LOGGING: add logging here DRIVER_LOG("fillLevel",-1,fillLevel);
  return fillLevel;
}
...
uint32_t FunctionGeneratorImpl::ReadExecutedParameterCount()
{
  ...
  if ( running )
  {
    ...
    // LOGGING: add logging here DRIVER_LOG("running_count",-1,count);
    return count;
  }
  else
  {
    // LOGGING: add logging here DRIVER_LOG("notrunning_count",-1,executedParameterCount);
...
{
  // LOGGING: add logging here DRIVER_LOG("",-1,-1);
  assert( channel == -1 );

  unsigned i;
  for ( i = 0; i < allocation->size(); ++i )
    if ( allocation->operator[]( i ) == -1 )
      break;
  ...
  channel                     = i;
  enabled                     = true;
  // LOGGING: add logging here DRIVER_LOG("got_channel",-1,channel);
...
void FunctionGeneratorImpl::releaseChannel()
{
  ...
  if ( abort )
  {
    // LOGGING: add logging here DRIVER_LOG("reset_firmware_channel",-1,channel);
  ...
  if ( abort )
  {
    // LOGGING: add logging here DRIVER_LOG("flush_channel",-1,previous_channel);
    flush();
  }
}
...
void FunctionGeneratorImpl::arm()
{
  ...
  if ( enabled )
  {
    // LOGGING: add logging here DRIVER_LOG("throw_enabled_cannot_rearm_channel",-1,channel);
    throw saftbus::Error( saftbus::Error::INVALID_ARGS, "Enabled, cannot re-Arm" );
  }
  if ( fillLevel == 0 )
  {
    // LOGGING: add logging here DRIVER_LOG("throw_filllevel_zero_channel",-1,channel);
  ...
  try
  {
    // LOGGING: add logging here DRIVER_LOG("refill_channel",-1,channel);
    refill( true );
    mbx->UseSlot( mb_slot, SWI_ENABLE | channel );
  }
  catch ( ... )
  {
    // LOGGING: add logging here DRIVER_LOG("refill_failed_channel",-1,channel);
    std::cerr << "FunctionGenerator: failed to fill buffers and send enabled SWI on channel " << std::dec << channel
...
void FunctionGeneratorImpl::Reset()
{
  ...
  if ( abort )
  {
    // LOGGING: add logging here DRIVER_LOG("abort alreay in progress",-1,channel);
  ...
  if ( !resetTimeout.connected() )
  {
    ...
    resetTimeout = saftbus::Loop::get_default().connect<saftbus::TimeoutSource>(
        std::bind( &FunctionGeneratorImpl::ResetFailed, this ),
