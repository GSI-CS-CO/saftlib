...
namespace saftlib {
  ...
  uint32_t BurstGenerator::readFirmwareId()
  {
    ...
    clog << kLogDebug << "BurstGenerator: method call readFirmwareId() succeeded." << std::endl;
  ...
  int32_t BurstGenerator::sendPulseParams(uint32_t eventHi, uint32_t delay, uint32_t conditions, uint32_t offset)
  {
    ...
    try
    {
      ...
      device.write(bg_slot, EB_DATA32, CMD_GET_PARAM); // load pulse parameters

      clog << kLogDebug << "BurstGenerator: method call sendPulseParams(" << eventHi << ", " <<
        delay << ", " << conditions << ", " << offset << ") succeeded." << std::endl;
      return EB_OK;
    }
  ...
  int32_t BurstGenerator::sendProdCycles(uint32_t eventHi, uint64_t cycles)
  {
    ...
    try
    {
      ...
      // instruct LM32 to obtain the argument data from the shared memory
      device.write(bg_slot, EB_DATA32, CMD_GET_CYCLE); // load production cycles

      clog << kLogDebug << "BurstGenerator: method call sendProdCycle(" << eventHi << ", " << cycles << ") succeeded." << std::endl;
      return EB_OK;
    }
    catch (etherbone::exception_t e)
    {
